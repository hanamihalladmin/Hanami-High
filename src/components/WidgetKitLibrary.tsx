...
import type { CreatorWidgetKit,CreatorWidgetKitFavorite } from '../types/database-widget-kits'
...
export function WidgetKitLibrary({onSearch,onNotifications,unreadCount}:Props){
 ...
 const load=useCallback(async()=>{
  ...
  const [hub,creatorResult,kitResult,favoriteResult,widgetResult]=await Promise.all([
  ...
  const first=hub.error||creatorResult.error||kitResult.error||favoriteResult.error||widgetResult.error
  if(first){setLoading(false);setError(first.message);return}
  setSnapshot(hub.data?.[0]??null);setCreators(creatorResult.data??[]);setKits(kitResult.data??[]);setFavorites(favoriteResult.data??[]);setWidgets(widgetResult.data??[]);setSelected(current=>current.filter(id=>(widgetResult.data??[]).some(widget=>widget.id===id)));setLoading(false)
 ...
 async function installKit(kit:CreatorWidgetKit){
  ...
  setWorking(null);if(result.error)return setError(result.error.message)
  const count=result.data?.[0]?.inserted_count??0;setNotice(`${count} widgets from ${kit.title} were added below your current Profile Studio layout.`);await load()
 }
