...
export function SettingsPage({ mode, onSearch, onNotifications, unreadCount }: Props) {
  ...
  const handleChangedAt = activeCharacter?.handle_changed_at ?? null
  ...
  async function changeHandle() {
    ...
    setWorking(true); setError(null); setNotice(null)
    const { error: handleError } = await client.rpc('change_character_handle', {
      p_character_id: activeCharacter.id,
      p_handle: normalizedHandle,
    })
    setWorking(false)
    if (handleError) return setError(handleError.message)
    setNotice(`Your character handle is now @${normalizedHandle}.`)
    ...
    return <div className="settings-stack">
      <section className="settings-account-board">
        <article className="settings-identity-card">
          <span className="settings-scope-tag">ACCOUNT</span>
          <div className="settings-account-avatar">{(currentAccount.discord_username || 'H').slice(0, 2).toUpperCase()}</div>
          <div><span className="eyebrow">DISCORD IDENTITY</span><h2>{currentAccount.discord_username || 'Connected Discord account'}</h2><p>This login owns both character slots and all account-wide Hanami benefits.</p></div>
        </article>
      ...
      <section className="settings-panel">
        <header><div><span className="eyebrow">ACCOUNT VS. CHARACTER</span><h2>Your Hanami identity map</h2></div><span className="settings-scope-tag">1 ACCOUNT · MAX 2 CHARACTERS</span></header>
        <div className="settings-scope-grid">
          <article><b>Account-wide</b><p>Discord login, Petals, Hanami+, Boutique inventory, interface preferences, account privacy, and moderation status.</p></article>
          <article><b>Character-specific</b><p>Handle, school role, profile, friends, posts, messages, classes, attendance, grades, clubs, and character achievements.</p></article>
        </div>
      </section>

      <section className="settings-panel">
        <header><div><span className="eyebrow">CURRENT CAMPUS IDENTITY</span><h2>{currentName}</h2></div><strong>@{currentCharacter.handle || 'no_handle'}</strong></header>
        <div className="settings-detail-list"><div><span>Character slot</span><strong>{currentCharacter.slot_no} of 2</strong></div><div><span>School role</span><strong>{characterLabel(currentCharacter.school_role, currentCharacter.character_kind)}</strong></div><div><span>Campus access</span><strong>{currentCharacter.character_state}</strong></div></div>
      </section>
  ...
  function renderCharacter() {
    return <div className="settings-stack">
      <section className="settings-panel">
        <header><div><span className="eyebrow">CURRENT CHARACTER</span><h2>{currentName}</h2></div><span className="settings-scope-tag">CHARACTER SLOT {currentCharacter.slot_no}</span></header>
        <div className="settings-detail-list"><div><span>School role</span><strong>{characterLabel(currentCharacter.school_role, currentCharacter.character_kind)}</strong></div><div><span>Status</span><strong>{currentCharacter.character_state}</strong></div><div><span>Current handle</span><strong>@{currentCharacter.handle || 'not-set'}</strong></div></div>
      </section>

      <section className="settings-panel handle-settings-panel">
        <header><div><span className="eyebrow">HANAMI HANDLE</span><h2>Your @handle</h2></div><strong>{handleLocked ? '7-DAY COOLDOWN' : 'EDITABLE'}</strong></header>
        <div className="handle-settings-body">
          <div className="handle-preview"><span>@</span><strong>{normalizedHandle || 'your_handle'}</strong></div>
          <label><span>Choose a unique handle</span><div className="handle-edit-row"><span className="handle-prefix">@</span><input value={handleDraft} disabled={working || handleLocked} maxLength={24} autoCapitalize="none" autoCorrect="off" spellCheck={false} onChange={(event) => setHandleDraft(event.target.value.toLowerCase().replace(/[^a-z0-9_]/g, ''))}/><button className="primary-action" type="button" disabled={working || handleLocked || !handleValid || !handleChanged} onClick={() => void changeHandle()}>{working ? 'Saving…' : 'Change handle'}</button></div></label>
          <div className={`handle-rule-note ${handleValid || !handleDraft ? '' : 'invalid'}`}>3–24 lowercase letters, numbers, and underscores. Handles are unique across Hanami.</div>
          {handleLocked && nextHandleChangeAt && <div className="settings-info-note">You can change this handle again after <strong>{formatTokyoDate(nextHandleChangeAt.toISOString())}</strong>. Staff with account-management authority can override the cooldown when necessary.</div>}
          {!handleLocked && handleChangedAt && <div className="settings-info-note">Changing your handle starts a new seven-day cooldown and automatically updates the handle shown on your published profile.</div>}
        </div>
      </section>
  ...
  function renderConnections() {
    return <div className="settings-stack">
      <section className="settings-panel"><header><div><span className="eyebrow">AUTHENTICATION</span><h2>Discord</h2></div><strong>CONNECTED</strong></header><div className="connection-card"><div className="connection-mark">D</div><div><strong>{currentAccount.discord_username || 'Discord account'}</strong><span>Discord is the authentication identity for this Hanami account. Hanami does not use a separate email login.</span></div></div></section>
      <section className="settings-panel"><header><div><span className="eyebrow">CAMPUS NETWORK</span><h2>Hanami High</h2></div><strong>ACTIVE</strong></header><div className="connection-card"><div className="connection-mark">H</div><div><strong>Hanami account</strong><span>Account state: {currentAccount.account_state}. This account owns your shared wallet, benefits, inventory, settings, and up to two characters.</span></div></div></section>
      <div className="settings-info-note">There are no other external services connected right now. This page will only show integrations after they actually exist.</div>
    </div>
  }

  return <main className="content-area settings-page">
    <ShellTopbar eyebrow="SETTINGS" title={meta[mode].title} onSearch={onSearch} onNotifications={onNotifications} unreadCount={unreadCount}/>
    <div className="settings-intro"><div><span className="eyebrow">HANAMI CONTROL CENTER</span><p>{meta[mode].description}</p></div><div className="settings-intro-identity"><span className="settings-scope-tag">{meta[mode].scope}</span><strong>{currentName}</strong><small>@{currentCharacter.handle || 'not-set'}</small></div></div>
    {error && <div className="identity-notice error">{error}</div>}
    {notice && <div className="identity-notice success">{notice}</div>}
    {loading ? <div className="settings-empty">Loading settings…</div> : mode === 'account' ? renderAccount() : mode === 'character' ? renderCharacter() : mode === 'privacy-safety' ? renderPrivacy() : mode === 'notifications' ? renderNotifications() : mode === 'accessibility' ? renderAccessibility() : renderConnections()}
  </main>
}
