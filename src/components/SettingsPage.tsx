...
export function SettingsPage({ mode, onSearch, onNotifications, unreadCount }: Props) {
  ...
  function renderAccount() {
    return <div className="settings-stack">
      <section className="settings-panel"><header><div><span className="eyebrow">HANAMI ACCOUNT</span><h2>Account identity</h2></div><strong>{currentAccount.account_state}</strong></header><div className="settings-detail-list"><div><span>Discord member</span><strong>{currentAccount.discord_username || 'Connected Discord account'}</strong></div><div><span>Character slots</span><strong>{characters.length} / 2 used</strong></div><div><span>Current character</span><strong>{currentCharacter.display_name || currentCharacter.first_name || `Slot ${currentCharacter.slot_no}`}</strong></div></div></section>
  ...
  function renderCharacter() {
    return <section className="settings-panel"><header><div><span className="eyebrow">CURRENT CHARACTER</span><h2>{currentCharacter.display_name || currentCharacter.first_name || 'Hanami Character'}</h2></div><strong>{characterLabel(currentCharacter.school_role, currentCharacter.character_kind)}</strong></header><div className="settings-detail-list"><div><span>Character slot</span><strong>{currentCharacter.slot_no} of 2</strong></div><div><span>School role</span><strong>{characterLabel(currentCharacter.school_role, currentCharacter.character_kind)}</strong></div><div><span>Status</span><strong>{currentCharacter.character_state}</strong></div></div>{characterPrefs && <div className="settings-form-list"><label><div><strong>Default post audience</strong><span>Used as the default when writing new social content.</span></div><select disabled={working} value={characterPrefs.default_post_visibility} onChange={(event) => void saveCharacter({ default_post_visibility: event.target.value })}><option value="hanami">Hanami Network</option><option value="friends">Friends</option><option value="private">Private</option></select></label><label><div><strong>Show school role on profile</strong><span>Display Student/Teacher status in eligible profile areas.</span></div><input type="checkbox" checked={characterPrefs.show_school_role} disabled={working} onChange={(event) => void saveCharacter({ show_school_role: event.target.checked })}/></label></div>}<div className="settings-info-note">Approved character identity fields are protected here. Use the Profile Studio for profile presentation; school-record identity changes remain controlled by Hanami workflows.</div></section>
  }
  ...
  function renderConnections() {
    return <div className="settings-stack"><section className="settings-panel"><header><div><span className="eyebrow">CONNECTED LOGIN</span><h2>Discord</h2></div><strong>Connected</strong></header><div className="connection-card"><div className="connection-mark">D</div><div><strong>{currentAccount.discord_username || 'Discord account'}</strong><span>Discord is the authentication identity for this Hanami account.</span></div></div></section><section className="settings-panel"><header><div><span className="eyebrow">HANAMI SERVICE</span><h2>School Network</h2></div><strong>Active</strong></header><div className="connection-card"><div className="connection-mark">H</div><div><strong>Hanami High account</strong><span>Account state: {currentAccount.account_state}. Character identity and Discord authentication stay separate.</span></div></div></section><div className="settings-info-note">No other external services are connected. Future staff or school integrations will appear here only after they are actually implemented.</div></div>
  }

  return <main className="content-area settings-page"><ShellTopbar eyebrow="SETTINGS" title={meta[mode].title} onSearch={onSearch} onNotifications={onNotifications} unreadCount={unreadCount}/><div className="settings-intro"><div><span className="eyebrow">HANAMI SETTINGS</span><p>{meta[mode].description}</p></div><strong>{currentCharacter.display_name || currentCharacter.first_name || 'Current character'}</strong></div>{error && <div className="identity-notice error">{error}</div>}{notice && <div className="identity-notice success">{notice}</div>}{loading ? <div className="settings-empty">Loading settings…</div> : mode === 'account' ? renderAccount() : mode === 'character' ? renderCharacter() : mode === 'privacy-safety' ? renderPrivacy() : mode === 'notifications' ? renderNotifications() : mode === 'accessibility' ? renderAccessibility() : renderConnections()}</main>
}
