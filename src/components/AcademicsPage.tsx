...
const modeMeta: Record<Mode, { title: string; description: string }> = {
  ...
  assignments: { title: 'Assignments', description: 'Assigned work, submissions, and teacher assignment management.' },
  grades: { title: 'Grades', description: 'Scores, feedback, and teacher grading tools.' },
  attendance: { title: 'Attendance', description: 'Attendance history and teacher roll-taking tools.' },
}
...
export function AcademicsPage({ mode, targetId, onSearch, onNotifications, unreadCount }: Props) {
  ...
  const isTeacherCharacter = Boolean(
    activeCharacter
  ...
  function renderOverview() {
    return <>
      <div className="academic-stat-grid">
        <article><span>CLASSES</span><strong>{ownSections.length}</strong><small>{isTeacherCharacter ? 'assigned / visible sections' : 'active enrollments'}</small></article>
      ...
      <div className="academic-card-grid">
        {ownSections.length === 0 ? <div className="academic-empty">No class sections are visible to this character yet.</div> : ownSections.map((section) => {
          ...
          return <article className={`academic-class-card ${targetId === section.id ? 'targeted' : ''}`} id={`academic-${section.id}`} key={section.id}>
            ...
            <footer>{teacherRows.length ? teacherRows.map((item) => <span key={item.character_id}>{identityName(item.character_id)} · {item.staff_role}</span>) : <span>Teacher TBA</span>}</footer>
          </article>
        })}
      </div>
