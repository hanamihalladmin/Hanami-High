  ...
  return (
    <div className="user-panel">
      <a className="user-panel-identity" href="#/profile/view-profile" title="View your profile">
        <div className="avatar-placeholder">{initials}<span className={`presence-dot status-${presence}`} /></div>
      ...
      <div className="user-panel-controls">
        <details className="presence-menu">
          <summary title="Set your status" aria-label={`Set status. Current status: ${presence}`}><span className={`presence-control-dot status-${presence}`} /></summary>
          <div className="presence-menu-popover">
            <span className="user-menu-label">SET STATUS</span>
            {presenceOptions.map((option) => <button className={presence === option.value ? 'selected' : ''} type="button" key={option.value} onClick={() => choosePresence(option.value)}>
              <span className={`presence-option-dot status-${option.value}`} />
              <span><strong>{option.label}</strong><small>{option.description}</small></span>
              {presence === option.value && <b>✓</b>}
            </button>)}
          </div>
        </details>
        ...
        <details className="user-menu">
          ...
          <div className="user-menu-popover">
            ...
            {characters.map((character) => (
              <button key={character.id} type="button" disabled={mutating || character.character_state !== 'active' || character.id === activeCharacter.id} onClick={() => void selectCharacter(character.id)}>
                {character.id === activeCharacter.id ? '✓ ' : ''}{characterName(character)}{' · '}{character.character_state === 'active' ? roleLabel(character) : character.character_state}
              </button>
            ))}
            <button type="button" disabled={mutating} onClick={() => void clearActiveCharacter()}>Character selection…</button>

            {(isOwner || isPlatformAdmin) && <div className="user-menu-rule" />}
            {isOwner && <><span className="user-menu-label">OWNER</span><button type="button" onClick={enterOwnerMode}>Open Owner Console</button></>}
            {isPlatformAdmin && <><span className="user-menu-label">ADMINISTRATOR</span><button type="button" onClick={enterAdminMode}>Open Administrator Console</button></>}
