...
export function OwnerLockscreenPanel(){
  ...
  async function copyPin(){if(!issued)return;await navigator.clipboard.writeText(issued.pin);setNotice('PIN copied. Save it in the Owner tracker and share it with the member through your approved private channel.')}

  return <section className="owner-lockscreen-panel">
    <header className="owner-lockscreen-heading"><div><span className="eyebrow">ACCOUNT LOCK SCREENS</span><h2>Owner PIN Console</h2><p>Generate or rotate six-digit member PINs. Every currently issued PIN is guaranteed unique across Hanami accounts. Students can customize only their wallpaper; they cannot create or change these PINs.</p></div><button type="button" onClick={()=>void load()} disabled={loading||Boolean(working)}>Refresh</button></header>
    {error&&<div className="identity-notice error">{error}</div>}{notice&&<div className="identity-notice success">{notice}</div>}
    {issued&&<section className="owner-issued-pin"><span className="eyebrow">SHOW ONCE · UNIQUE NEW PIN</span><h3>{issued.username}</h3><strong>{issued.pin}</strong><p>This PIN is unique across all currently issued member PINs. The readable value is only returned at generation time; Hanami stores a one-way hash after this.</p><div><button className="primary-action" type="button" onClick={()=>void copyPin()}>Copy PIN</button><button className="secondary-action" type="button" onClick={()=>setIssued(null)}>I’ve saved it</button></div></section>}
