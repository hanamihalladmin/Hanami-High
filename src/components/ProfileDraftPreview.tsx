...
export function ProfileDraftPreview({ onClose }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    const [profileResult, widgetResult, loadoutResult, itemResult] = await Promise.all([
      ...
      client.from('profile_widgets').select('*').eq('character_id', activeCharacter.id).eq('is_visible', true).order('y').order('x'),
  ...
  const sidebar = <aside className="hanami-profile-about">
    ...
    {layout.showSchoolIdentity && <section><span className="eyebrow">SCHOOL IDENTITY</span><dl><div><dt>Role</dt><dd>{roleLabel(activeCharacter.school_role)}</dd></div><div><dt>Network</dt><dd>Hanami High</dd></div></dl></section>}
    <section><span className="eyebrow">PREVIEW NOTE</span><p>This is your private draft. Guestbook, activity, and visitor-only controls are not simulated here.</p></section>
  </aside>

  return <div className="profile-draft-preview-backdrop" role="dialog" aria-modal="true" aria-label="Private profile draft preview">
    <div className="profile-draft-preview-window">
      <header className="profile-draft-preview-toolbar">
        <div><span className="eyebrow">PRIVATE DRAFT PREVIEW</span><strong>{name}</strong><small>Nothing here is public until you press Publish Profile.</small></div>
      ...
      {loading || !profile ? <div className="studio-loading">Rendering your private profile draft…</div> : <div className="profile-draft-preview-scroll">
        <div className={`hanami-profile-shell spacehey-profile-page layout-${layout.preset} sidebar-${layout.sidebarSide} hero-${layout.heroStyle} density-${layout.panelDensity} titles-${layout.widgetTitleStyle} avatar-${layout.avatarShape} tabs-${layout.tabStyle} ${theme.grid ? 'show-grid' : ''} cosmetic-frame-${safeClass(cosmetics.frame)} cosmetic-avatar-${safeClass(cosmetics.avatarDecoration)} cosmetic-effect-${safeClass(cosmetics.effect)} cosmetic-card-${safeClass(cosmetics.profileCard)} cosmetic-bg-${safeClass(cosmetics.backgroundPack)}`} style={style}>
          <section className="hanami-profile-hero">
            ...
            <div className="hanami-profile-heading"><div><span className="eyebrow">HANAMI PROFILE</span><h1 className={`profile-display-name profile-font-${theme.displayFont} profile-effect-${theme.displayEffect}`}>{name}</h1><p>{handle}{profile.pronouns ? ` · ${profile.pronouns}` : ''}</p>{profile.custom_status && <blockquote>{profile.custom_status}</blockquote>}</div><div className="hanami-profile-actions"><button className="primary-action" type="button" disabled>Edit Profile</button><button className="secondary-action" type="button" disabled>Name Style</button></div></div>
          </section>
          <div className="hanami-profile-layout">
            {layout.sidebarSide === 'left' && sidebar}
            <section className="hanami-profile-main">
              <nav className="hanami-profile-tabs"><button className="active" type="button">Profile Board</button><button type="button" disabled>Activity</button><button type="button" disabled>Guestbook</button></nav>
