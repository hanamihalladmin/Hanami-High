...
const animatedNames=new Set(['Petal Drift','Petal Shower','Sakura Drift Halo','Star Spark','Starlight Trail','Starlight Sprites','Bubble Pop','Firefly Drift','Arcade Bloom','Midnight Shrine','Aurora Mist','Moonlit Koi Orbit','Crystal Bloom Frame','Gilded Moon Frame','Aurora Wing Frame'])
const categoryLinks=[
...
export function BoutiquePage({mode,onSearch,onNotifications,unreadCount}:Props){
 ...
 const collections=useMemo(()=>{const map=new Map<string,BoutiqueItem[]>();visible.forEach(item=>{const key=item.collection_name||'Hanami Collection';map.set(key,[...(map.get(key)||[]),item])});return [...map.entries()]},[visible])

 async function purchase(item:BoutiqueItem){const client=supabase;if(!client)return;setWorking(item.id);setError(null);setNotice(null);const {data,error:e}=await client.rpc('purchase_boutique_item',{p_item_id:item.id,p_request_id:crypto.randomUUID()});setWorking(null);if(e)return setError(e.message);setNotice(item.item_type==='hanami_plus_pass'?`${item.name} extended your Hanami+ access.`:`${item.name} is now in your account inventory. ${data?.[0]?.balance??wallet?.balance??0} Petals remaining.`);await load()}
 async function toggleWish(item:BoutiqueItem){const client=supabase;if(!client||!account)return;setError(null);if(wished.has(item.id)){const {error:e}=await client.from('boutique_wishlist').delete().eq('account_id',account.id).eq('item_id',item.id);if(e)return setError(e.message);setWishlist(current=>current.filter(row=>row.item_id!==item.id))}else{const {data,error:e}=await client.from('boutique_wishlist').insert({account_id:account.id,item_id:item.id}).select('*').single();if(e)return setError(e.message);setWishlist(current=>[data,...current])}}
 function buyButton(item:BoutiqueItem,compact=false){const row=owned.get(item.id),enough=(wallet?.balance??0)>=item.price_petals,repeat=item.item_type==='hanami_plus_pass',can=enough&&(!row||repeat);if(mode==='my-inventory'&&catalogView==='mode')return equippableTypes.has(item.item_type)?<a className="boutique-v3-equip" href="#/profile/profile-studio" onClick={e=>e.stopPropagation()}>Equip</a>:<span className="boutique-v3-owned">Owned ×{row?.quantity??1}</span>;return <button className="boutique-v3-buy" disabled={!can||working===item.id} onClick={e=>{e.stopPropagation();void purchase(item)}}>{working===item.id?'Purchasing…':row&&!repeat?'Owned':!enough?'Need more Petals':compact?`❀ ${item.price_petals}`:`Buy for ❀ ${item.price_petals}`}</button>}
 function heart(item:BoutiqueItem){return <button className={`boutique-v3-heart ${wished.has(item.id)?'wished':''}`} type="button" aria-label={wished.has(item.id)?'Remove from wishlist':'Add to wishlist'} onClick={e=>{e.stopPropagation();void toggleWish(item)}}>{wished.has(item.id)?'♥':'♡'}</button>}
 function card(item:BoutiqueItem){const row=owned.get(item.id);return <article className={`boutique-v3-card rarity-${item.rarity} ${hero?.id===item.id?'selected':''}`} key={item.id} onClick={()=>setPreviewId(item.id)}><div className="boutique-v3-card-art"><BoutiqueArtworkV2 name={item.name} type={item.item_type}/>{heart(item)}<div className="boutique-v3-flags">{item.is_new&&<span>NEW</span>}{animatedNames.has(item.name)&&<span>ANIMATED</span>}{row&&<span>OWNED</span>}</div></div><div className="boutique-v3-card-copy"><span className="boutique-v3-collection">{item.collection_name||item.season}</span><h3>{item.name}</h3><p>{item.description}</p><div className="boutique-v3-meta"><span>{typeLabels[item.item_type]||item.item_type}</span><span>{item.rarity}</span></div></div><footer><strong>{row?'In inventory':`❀ ${item.price_petals}`}</strong>{buyButton(item,true)}</footer></article>}
 ...
 return <main className="content-area boutique-page boutique-v3">
  ...
  <section className="boutique-v3-shell">
   ...
   {loading?<div className="boutique-v3-empty">Loading the Boutique…</div>:<>
    {hero&&<section className={`boutique-v3-hero rarity-${hero.rarity}`}><div className="boutique-v3-hero-art"><BoutiqueArtworkV2 name={hero.name} type={hero.item_type} large/>{heart(hero)}{animatedNames.has(hero.name)&&<span className="boutique-v3-animated-chip">✦ ANIMATED</span>}</div><div className="boutique-v3-hero-copy"><div className="boutique-v3-meta"><span>{typeLabels[hero.item_type]||hero.item_type}</span><span>{hero.rarity}</span>{hero.is_new&&<span>new</span>}</div><span className="eyebrow">{hero.collection_name||hero.season}</span><h2>{hero.name}</h2><p>{hero.description}</p><div className="boutique-v3-price"><strong>❀ {hero.price_petals}</strong>{hero.pass_days&&<small>{hero.pass_days} days of Hanami+</small>}</div><div className="boutique-v3-actions">{buyButton(hero)}<button type="button" onClick={()=>void toggleWish(hero)}>{wished.has(hero.id)?'♥ Wishlisted':'♡ Add to wishlist'}</button></div><div className="boutique-v3-ownership"><span>Account owns purchases</span><span>Character equips cosmetics</span>{plusActive&&<span>Hanami+ active</span>}</div></div></section>}
