...
export function OwnerAccessPreview() {
  ...
  const [workspace, setWorkspace] = useState<OwnerWorkspace>('school')
      ...
      <section className="identity-window owner-window">
        ...
        <div className="identity-body owner-body">
          <div className="owner-mode-banner">
            ...
            <span>You are operating as the Hanami account owner, not as an RP character. School administration is account-level; portal previews remain read-only and clearly labeled.</span>
          </div>

          <div className="owner-summary-grid">
            <article><span>ACCOUNT</span><strong>{account?.discord_username || 'Connected owner'}</strong><small>{account?.account_state || 'active'}</small></article>
            <article><span>OWN CHARACTERS</span><strong>{characters.length} / 2</strong><small>Owner access is independent of character slots.</small></article>
            <article><span>CAPABILITIES</span><strong>{capabilities.length}</strong><small>Account-level Owner permissions loaded from Supabase.</small></article>
          </div>
