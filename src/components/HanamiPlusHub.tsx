...
import type {
  ...
  HanamiPlusDropChoice,
  HanamiPlusDropOffer,
  HanamiPlusDropOfferItem,
  HanamiPlusGiftRow,
  HanamiPlusHubSnapshot,
  HanamiPlusLoyaltyCatalogRow,
  HanamiPlusRewardHistoryRow,
} from '../types/database-rewards'
import type { HanamiPlusMonthlyReward } from '../types/database-hanami-plus'
...
export function HanamiPlusHub({ mode, onSearch, onNotifications, unreadCount }: Props) {
  ...
  const [monthlyReward, setMonthlyReward] = useState<HanamiPlusMonthlyReward | null>(null)
  const [dropOffers, setDropOffers] = useState<HanamiPlusDropOffer[]>([])
  const [dropOfferItems, setDropOfferItems] = useState<HanamiPlusDropOfferItem[]>([])
  const [dropChoices, setDropChoices] = useState<HanamiPlusDropChoice[]>([])
  ...
  const load = useCallback(async () => {
    ...
    const [hubResult, historyResult, giftResult, eventResult, loyaltyResult, betaResult, prefResult, monthlyResult, offerResult, offerItemsResult, choiceResult] = await Promise.all([
      ...
      client.from('hanami_plus_monthly_rewards').select('*').eq('claim_month', tokyoMonthStart()).maybeSingle(),
      client.from('hanami_plus_drop_offers').select('*').order('starts_at'),
      client.from('hanami_plus_drop_offer_items').select('*').order('sort_order'),
      client.from('hanami_plus_drop_choices').select('*').eq('account_id', account.id),
    ])

    const firstError = [hubResult.error, historyResult.error, giftResult.error, eventResult.error, loyaltyResult.error, betaResult.error, prefResult.error, monthlyResult.error, offerResult.error, offerItemsResult.error, choiceResult.error].find(Boolean)
    if (firstError) {
    ...
    setMonthlyReward(monthlyResult.data)
    setDropOffers(offerResult.data ?? [])
    setDropOfferItems(offerItemsResult.data ?? [])
    setDropChoices(choiceResult.data ?? [])
    ...
    const itemIds = Array.from(new Set([
      ...
      ...(monthlyResult.data ? [monthlyResult.data.boutique_item_id] : []),
      ...(offerItemsResult.data ?? []).map((row) => row.boutique_item_id),
    ]))
    if (itemIds.length) {
  ...
  const recentInbox = useMemo(() => {
    const rewardRows = history.slice(0, 8).map((row) => ({ id: `reward:${row.id}`, when: row.occurred_at, title: row.title, body: row.description || kindLabel(row.reward_kind), kind: 'Reward' }))
  ...
  async function toggleBeta(feature: HanamiPlusBetaFeature, optedIn: boolean) {
    const client = supabase
    if (!client) return
    setWorking(feature.feature_key); setError(null); setNotice(null)
    const { error: toggleError } = await client.rpc('set_hanami_plus_beta_preference', { p_feature_key: feature.feature_key, p_opted_in: optedIn })
    setWorking(null)
  ...
  async function claimMonthly() {
    const client = supabase
    if (!client) return
    setWorking('monthly'); setError(null); setNotice(null)
    const { data, error: claimError } = await client.rpc('claim_hanami_plus_monthly_reward')
    setWorking(null)
    if (claimError) return setError(claimError.message)
    setNotice(`${data?.[0]?.item_name || 'Monthly cosmetic'} added to your account inventory.`)
    await load()
  }

  async function chooseDrop(offerId: string, itemId: string) {
    const client = supabase
    if (!client) return
    setWorking(`drop:${offerId}`); setError(null); setNotice(null)
    const { data, error: choiceError } = await client.rpc('choose_hanami_plus_drop', { p_offer_id: offerId, p_item_id: itemId })
    setWorking(null)
    if (choiceError) return setError(choiceError.message)
    setNotice(`${data?.[0]?.item_name || 'Drop cosmetic'} added to your account inventory.`)
    await load()
  }

  async function buyLoyalty(itemId: string) {
    const client = supabase
    if (!client) return
    setWorking(`loyalty:${itemId}`); setError(null); setNotice(null)
    const { data, error: purchaseError } = await client.rpc('purchase_hanami_plus_loyalty_item', { p_item_id: itemId, p_request_id: crypto.randomUUID() })
    setWorking(null)
    if (purchaseError) return setError(purchaseError.message)
    setNotice(`${data?.[0]?.item_name || 'Loyalty cosmetic'} purchased. New balance: ❀ ${data?.[0]?.balance ?? snapshot?.petal_balance ?? 0}.`)
    await load()
  }
  ...
  return (
    <main className="hanami-plus-page">
      ...
      <section className="plus-hero">
        <div><span className="eyebrow">CREATIVE MEMBERSHIP</span><h1>{active ? 'Hanami+ is blooming.' : 'Make Hanami more yours.'}</h1><p>{page.description}</p><p className="plus-core-note">Hanami+ is optional. Classes, clubs, roleplay, messaging, and the core Hanami community remain available without it.</p></div>
        <div className={`plus-status-card ${active ? 'active' : ''}`}><span>{active ? 'ACTIVE' : 'STANDARD'}</span><strong>{active ? `${snapshot?.days_remaining ?? 0} days` : 'Hanami'}</strong><small>{active ? `through ${formatDate(snapshot?.ends_at)}` : 'Earn or redeem Plus when you want the creative extras.'}</small></div>
      </section>
      ...
      {loading ? <section className="plus-empty"><strong>Opening your Hanami+ hub…</strong></section> : <>
        {mode === 'overview' && <>
          <section className="plus-stat-grid">
            ...
            <article className="plus-claim-card"><span>MONTHLY CLAIM</span><strong>{snapshot?.monthly_claimed ? 'Claimed' : monthlyReward ? itemById.get(monthlyReward.boutique_item_id)?.name || 'Monthly cosmetic' : 'Not scheduled'}</strong><small>{snapshot?.monthly_claimed ? 'This month’s reward is already in your inventory.' : monthlyReward ? 'One rotating cosmetic claim for active Hanami+.' : 'The owner has not published this month’s reward yet.'}</small>{monthlyReward && !snapshot?.monthly_claimed && <button type="button" disabled={!active || working === 'monthly'} onClick={() => void claimMonthly()}>{working === 'monthly' ? 'Claiming…' : active ? 'Claim monthly cosmetic' : 'Hanami+ required'}</button>}</article>
          </section>

          {dropOffers.length > 0 && <section className="plus-panel plus-drop-panel"><header><div><span className="eyebrow">CHOOSE YOUR DROP</span><h2>Pick your event cosmetic</h2><p>Some Hanami+ events let you choose one reward instead of receiving a fixed item.</p></div></header><div className="plus-drop-grid">{dropOffers.map((offer) => { const chosen = dropChoices.find((choice) => choice.offer_id === offer.id); const choices = dropOfferItems.filter((row) => row.offer_id === offer.id); return <article key={offer.id}><span>{chosen ? 'CHOICE LOCKED' : 'ACTIVE OFFER'}</span><h3>{offer.title}</h3><p>{offer.description || 'Choose one cosmetic from this event.'}</p><div>{choices.map((choice) => { const item = itemById.get(choice.boutique_item_id); const selected = chosen?.boutique_item_id === choice.boutique_item_id; return <button type="button" key={choice.boutique_item_id} disabled={!active || Boolean(chosen) || working === `drop:${offer.id}`} onClick={() => void chooseDrop(offer.id, choice.boutique_item_id)}>{selected ? '✓ ' : ''}{item?.name || 'Event cosmetic'}</button> })}</div></article> })}</div></section>}

          <section className="plus-overview-grid">
            <article className="plus-panel"><header><div><span className="eyebrow">COMING UP</span><h2>Plus Calendar</h2></div><a href="#/hanami-plus/calendar">View calendar →</a></header><div className="plus-list">{upcomingEvents.length ? upcomingEvents.map((event) => <div key={event.id} className="plus-list-row"><span>{kindLabel(event.event_kind)}</span><strong>{event.title}</strong><small>{formatDate(event.starts_at)}</small></div>) : <p className="plus-muted">No Plus events have been published yet.</p>}</div></article>
            <article className="plus-panel"><header><div><span className="eyebrow">RECENT</span><h2>Rewards & Gifts</h2></div><a href="#/hanami-plus/inbox">Open inbox →</a></header><div className="plus-list">{recentInbox.length ? recentInbox.slice(0, 5).map((entry) => <div key={entry.id} className="plus-list-row"><span>{entry.kind}</span><strong>{entry.title}</strong><small>{entry.body}</small></div>) : <p className="plus-muted">Your Plus reward inbox is empty.</p>}</div></article>
          </section>

          <section className="plus-feature-map"><header><span className="eyebrow">APPROVED ROADMAP</span><h2>Your Hanami+ world</h2><p>The 129 approved features are being built as connected systems, not isolated perks.</p></header><div>{[
          ...
          ].map(([title, body]) => <article key={title}><strong>{title}</strong><p>{body}</p></article>)}</div></section>
        </>}
        ...
        {mode === 'loyalty-shop' && <section className="plus-panel plus-full-panel"><header><div><span className="eyebrow">CUMULATIVE PLUS DAYS</span><h2>Loyalty Shop</h2><p>Your lifetime Plus days unlock the right to buy special cosmetics with earned Petals.</p></div><strong>{snapshot?.lifetime_plus_days ?? 0} days</strong></header><div className="plus-loyalty-grid">{loyalty.length ? loyalty.map((row) => { const item = itemById.get(row.boutique_item_id); const unlocked = (snapshot?.lifetime_plus_days ?? 0) >= row.minimum_lifetime_days; const affordable = (snapshot?.petal_balance ?? 0) >= row.price_petals; return <article key={row.boutique_item_id} className={unlocked ? '' : 'locked'}><span>{unlocked ? 'UNLOCKED' : `${row.minimum_lifetime_days} DAYS REQUIRED`}</span><strong>{item?.name || 'Loyalty cosmetic'}</strong><p>{item?.description || 'Owner-configured Hanami+ loyalty reward.'}</p><small>❀ {row.price_petals} Petals</small><button type="button" disabled={!active || !unlocked || !affordable || working === `loyalty:${row.boutique_item_id}`} onClick={() => void buyLoyalty(row.boutique_item_id)}>{working === `loyalty:${row.boutique_item_id}` ? 'Purchasing…' : !active ? 'Hanami+ required' : !unlocked ? 'Locked' : !affordable ? 'Not enough Petals' : 'Purchase'}</button></article> }) : <p className="plus-muted">The owner has not published any loyalty-shop cosmetics yet.</p>}</div></section>}
