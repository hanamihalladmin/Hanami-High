...
const taskMeta: Record<string, TaskMeta> = {
  acceptance_letter: {
    title: 'Read your acceptance letter',
    description: 'Your official Hanami High acceptance letter starts your first day.',
    available: false,
  },
  student_identity: {
    title: 'Review your student identity',
    description: 'Confirm the character name and New Student status you are using around campus.',
    available: true,
    action: 'Identity reviewed',
  },
  schedule: {
    title: 'Review your class schedule',
    description: 'Your assigned classes and school-day timetable will live in Academics.',
    available: false,
  },
  network: {
    title: 'Explore the Hanami Network',
    description: 'Learn the Home, Messages, Social, Academics, Campus, Boutique, and Profile areas.',
    available: true,
    action: 'I know my way around',
  },
  ...
  boutique_preview: {
    ...
    description: 'Preview one cosmetic without being required to spend any Petals.',
    available: false,
  },
}
...
export function OrientationPanel() {
  ...
  const progress = useMemo(() => {
    ...
    const completed = required.filter((task) => task.completed_at)
  ...
  async function completeTask(taskCode: string) {
    ...
    const { error: taskError } = await client.rpc('complete_orientation_task', {
  ...
  if (loading) {
    return (
      <section className="orientation-card orientation-live">
        <div><span className="tag">🌱 NEW STUDENT</span><h2>Loading orientation…</h2></div>
      </section>
    )
  }
  ...
  if (!orientation) {
    return (
      <section className="orientation-card orientation-live">
        <div><span className="tag">🌱 NEW STUDENT</span><h2>Welcome to Hanami High</h2><p>Your orientation record is being prepared.</p></div>
      </section>
    )
  }
  ...
  return (
    <section className="orientation-live-panel">
      <header>
        <div>
          ...
          <p>
            {complete
              ? 'You are ready to explore Hanami on your own. Your New Student role remains until Student Affairs promotes you.'
              : orientation.track === 'returning'
                ? 'This is your returning-member orientation: only character-specific steps are required.'
                : 'Complete the required first-day steps as Hanami systems become available.'}
          </p>
        </div>
        <div className="orientation-progress-live">
          <strong>{progress.completed} / {progress.total}</strong>
          <span>{complete ? 'Complete' : 'required steps'}</span>
        </div>
      </header>
        ...
        {tasks.map((task) => {
          ...
          return (
            <article className={`${done ? 'done' : ''} ${task.required ? '' : 'optional'}`} key={task.task_code}>
              ...
              <div>
                <strong>{meta.title}{!task.required ? ' · Optional' : ''}</strong>
              ...
              {done ? (
                ...
                <button
                  type="button"
                  disabled={completing === task.task_code}
                  onClick={() => void completeTask(task.task_code)}
                >
                  {completing === task.task_code ? 'Saving…' : meta.action || 'Complete'}
                </button>
              )}
            </article>
          )
