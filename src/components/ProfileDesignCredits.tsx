...
export function ProfileDesignCredits(){
  ...
  const load=useCallback(async()=>{
    ...
    if(!client||!route.active||!route.characterId){setCredits([]);return}
    setLoading(true)
    const result=await client.rpc('profile_design_credits',{p_character_id:route.characterId})
    setLoading(false)
    if(result.error){setCredits([]);return}
    setCredits(result.data??[])
  },[route.active,route.characterId])
  ...
  return <aside className={`profile-design-credits ${open?'open':''}`} aria-label="Profile design credits">
    ...
    {open&&<div className="profile-design-credits-body">
      ...
      {theme&&<section className="design-credit-theme"><span className="design-credit-kind">FULL THEME SOURCE</span><h3>{theme.source_title}</h3><strong>{theme.source_creator}</strong><small>{theme.version_label} · applied {formatDate(theme.applied_at)}</small><p>{theme.attribution_text}</p><a href="#/hanami-plus/marketplace">Browse Theme Marketplace →</a></section>}
      {components.length>0&&<section><div className="design-credit-section-title"><span>CREATOR COMPONENTS</span><small>{components.reduce((sum,row)=>sum+row.item_count,0)} widgets</small></div><div className="design-credit-component-list">{components.map((credit,index)=><article key={`${credit.source_id}:${credit.version_label}:${index}`}><div><strong>{credit.source_title}</strong><small>{credit.source_creator} · {credit.version_label}</small></div><span>{credit.item_count} widget{credit.item_count===1?'':'s'}</span><p>{credit.attribution_text}</p></article>)}</div><a className="design-credit-library-link" href="#/hanami-plus/component-library">Browse Component Library →</a></section>}
