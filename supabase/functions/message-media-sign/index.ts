...
Deno.serve(async(req:Request)=>{
 ...
 const owned=new Set((await ownedResponse.json() as Array<{id:string}>).map(row=>row.id));
 if(!owned.size)return json({error:"No Hanami character"},403);

 const participantsResponse=await fetch(`${SUPABASE_URL}/rest/v1/conversation_participants?conversation_id=eq.${encodeURIComponent(body.conversation_id)}&select=character_id`,{headers:{apikey:SERVICE_ROLE_KEY,Authorization:`Bearer ${SERVICE_ROLE_KEY}`}});
 if(!participantsResponse.ok)return json({error:"Conversation access check failed"},500);
 const participants=await participantsResponse.json() as Array<{character_id:string}>;
 if(!participants.some(row=>owned.has(row.character_id)))return json({error:"Conversation access denied"},403);
 ...
 const sign=await fetch(`${SUPABASE_URL}/storage/v1/object/sign/message-media/${encodeURI(body.storage_path)}`,{method:"POST",headers:{apikey:SERVICE_ROLE_KEY,Authorization:`Bearer ${SERVICE_ROLE_KEY}`,"content-type":"application/json"},body:JSON.stringify({expiresIn:900})});
