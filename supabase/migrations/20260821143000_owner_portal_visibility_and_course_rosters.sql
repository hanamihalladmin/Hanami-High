...
begin
  ...
  select jsonb_build_object(
    ...
    'students',coalesce((select jsonb_agg(jsonb_build_object('display_name',c.display_name,'handle',c.handle) order by c.display_name) from public.section_memberships sm join public.characters c on c.id=sm.character_id where sm.section_id=cs.id and sm.relationship='student'),'[]'::jsonb),
...
create or replace function public.owner_portal_directory()
returns jsonb
language plpgsql
stable security definer
set search_path to 'public','private'
...
begin
  ...
  select jsonb_build_object(
    'characters',coalesce((select jsonb_agg(jsonb_build_object('id',c.id,'role',c.role,'display_name',c.display_name,'handle',c.handle,'visibility',c.visibility,'is_active',c.is_active,'created_at',c.created_at) order by c.role,c.display_name) from public.characters c),'[]'::jsonb),
    'memberships',coalesce((select jsonb_agg(jsonb_build_object('section_id',sm.section_id,'character_id',sm.character_id,'relationship',sm.relationship,'class_sections',jsonb_build_object('section_code',cs.section_code,'term',cs.term,'room',cs.room,'academic_courses',jsonb_build_object('code',ac.code,'title',ac.title)))) from public.section_memberships sm join public.class_sections cs on cs.id=sm.section_id join public.academic_courses ac on ac.id=cs.course_id),'[]'::jsonb)
  ) into result;
...
grant execute on function public.owner_portal_directory() to authenticated;
