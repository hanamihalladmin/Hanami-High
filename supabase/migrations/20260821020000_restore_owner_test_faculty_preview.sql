...
select c.id,'TST-A','2006-2007','TEST ROOM',12
from public.academic_courses c
where c.code='TEST-101'
on conflict (course_id,section_code,term) do update set
  room=excluded.room,
...
cross join (values
  (2::smallint,'09:15'::time,'10:00'::time),
  (4::smallint,'09:15'::time,'10:00'::time)
) as v(weekday,starts_at,ends_at)
where s.section_code='TST-A' and s.term='2006-2007'
on conflict (section_id,weekday,starts_at) do update set
...
begin
  ...
  if not exists (
    ...
    where c.id=target_character_id
      ...
      and c.role='faculty'
      and c.handle like 'testfaculty_%'
  ) then
    raise exception 'Only your Owner TEST Faculty character can use this fixture';
  end if;

  select s.id into target_section
  from public.class_sections s
  join public.academic_courses c on c.id=s.course_id
  where c.code='TEST-101'
    and s.section_code='TST-A'
    and s.term='2006-2007'
  limit 1;

  if target_section is null then
    raise exception 'TEST-101 preview section is unavailable';
  end if;
...
set search_path=public,private
as $$
  select private.attach_test_faculty_section_internal(target_character_id);
$$;
