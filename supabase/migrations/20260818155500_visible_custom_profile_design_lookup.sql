...
as $$
  select
    ...
    coalesce((select jsonb_agg(jsonb_build_object(
      'id',w.id,'widget_type',w.widget_type,'x',w.x,'y',w.y,'width',w.width,'height',w.height,
      'z_index',w.z_index,'rotation',w.rotation,'opacity',w.opacity,'content',w.content,'style',w.style,'locked',w.locked
    ) order by w.z_index,w.id) from public.character_profile_widgets w where w.character_id=target.id),'[]'::jsonb) as widgets
  ...
  where viewer.id=viewer_character_id
    ...
    and (target.owner_user_id=auth.uid() or target.visibility='public')
  limit 1;
$$;
revoke all on function private.visible_profile_design_internal(uuid,text) from public,anon;
grant execute on function private.visible_profile_design_internal(uuid,text) to authenticated;
...
as $$ select * from private.visible_profile_design_internal(viewer_character_id,target_handle); $$;
revoke all on function public.lookup_visible_profile_design(uuid,text) from public,anon;
