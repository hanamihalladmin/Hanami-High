  ...
  with input as (
    ...
    select
      ...
      greatest(
        case
          ...
          when lower(d.title) like p.normalized_query||'%' then 4.5
          when lower(coalesce(d.subtitle,'')) like p.normalized_query||'%' then 4.0
          when lower(d.title) like '%'||p.normalized_query||'%' then 3.5
          when lower(coalesce(d.subtitle,'')) like '%'||p.normalized_query||'%' then 3.0
          when lower(coalesce(d.body,'')) like '%'||p.normalized_query||'%' then 2.5
          else 0.0
        end,
    ...
    where length(p.raw_query)>0
      and (
        ...
        or lower(d.title) like '%'||p.normalized_query||'%'
        or lower(coalesce(d.subtitle,'')) like '%'||p.normalized_query||'%'
        or lower(coalesce(d.body,'')) like '%'||p.normalized_query||'%'
      )
  )
