...
test("test faculty character uses the signed-in account instead of shared credentials",()=>{assert.match(manager,/Create Test Faculty/);assert.match(manager,/\[TEST\] Faculty Preview/);assert.match(manager,/role:"faculty"/);assert.match(manager,/owner_user_id:userId/);assert.match(manager,/under this Discord account/);});
...
test("message history no longer blocks character deletion",()=>{assert.match(deletion,/on delete set null/);assert.match(deletion,/conversation_messages_sender_character_id_fkey/);assert.match(deletion,/conversations_created_by_character_id_fkey/);});
