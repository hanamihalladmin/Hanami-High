...
test("public announcements use live Supabase data with a labeled fallback",()=>{assert.match(liveAnnouncements,/apikey:SUPABASE_PUBLISHABLE_KEY/);assert.match(liveAnnouncements,/FALLBACK PREVIEW/);assert.match(liveAnnouncements,/is_test_data/);assert.doesNotMatch(liveAnnouncements,/service_role/i);});
test("public next event uses the live school calendar with a labeled fallback",()=>{assert.match(liveNextEvent,/apikey:SUPABASE_PUBLISHABLE_KEY/);assert.match(liveNextEvent,/Fallback preview/);assert.match(liveNextEvent,/is_test_data/);assert.match(liveNextEvent,/timeZone:"Asia\/Tokyo"/);});
