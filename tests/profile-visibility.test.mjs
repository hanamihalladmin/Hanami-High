...
test("public and accepted Friends-only profiles are visible while private stays owner only",()=>{
  assert.match(baseMigration,/target\.visibility = 'public'/);
  assert.match(friendshipMigration,/target\.visibility='friends_only'/);
  assert.match(friendshipMigration,/private\.characters_are_friends\(viewer\.id,target\.id\)/);
...
test("profile lookup keeps private helpers behind security-invoker wrappers",()=>{
  assert.match(baseMigration,/private\.visible_character_profile_internal/);
  assert.match(baseMigration,/create or replace function public\.lookup_visible_character_profile/);
  assert.match(baseMigration,/security invoker/);
  assert.match(friendshipMigration,/private\.visible_profile_design_internal/);
  assert.match(mediaMigration,/private\.can_view_character_profile_internal/);
  assert.match(mediaMigration,/security invoker/);
});
...
test("both role dashboards expose privacy-aware profile lookup",()=>{
  ...
  assert.match(dashboard,/FriendsPanel/);
});
