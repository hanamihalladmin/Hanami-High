...
test("campus keeps fixture directory data labeled while public opportunities use the live portal flow",()=>{assert.match(directory,/DEVELOPMENT DATA/);assert.match(campus,/LiveCampusOpportunities/);assert.match(opportunities,/Apply in Student Portal/);assert.match(opportunities,/status=eq\.published/);assert.match(campus,/Only published opportunities appear here/);assert.doesNotMatch(campus,/TEST LISTINGS/);});
