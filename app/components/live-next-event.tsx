...
export default function LiveNextEvent(){
 const [event,setEvent]=useState<EventRow|null>(null);const [roleplayNowMs,setRoleplayNowMs]=useState<number|null>(null);
 useEffect(()=>{let cancelled=false;const tick=()=>setRoleplayNowMs(hanamiRoleplayNow().getTime());tick();const clock=window.setInterval(tick,60000);async function load(){try{const now=encodeURIComponent(hanamiRoleplayNow().toISOString());const response=await fetch(`${SUPABASE_URL}/rest/v1/school_calendar_events?select=id,title,description,location,starts_at,all_day,category,featured&status=eq.published&starts_at=gte.${now}&order=featured.desc,starts_at.asc&limit=1`,{headers:{apikey:SUPABASE_PUBLISHABLE_KEY}});if(!response.ok)return;const rows=await response.json() as EventRow[];if(!cancelled)setEvent(rows[0]??null);}catch{/* Leave the empty live state visible. */}}load();return()=>{cancelled=true;window.clearInterval(clock);};},[]);
 ...
 const start=new Date(event.starts_at);const days=roleplayNowMs===null?0:Math.max(0,Math.ceil((start.getTime()-roleplayNowMs)/86400000));const when=new Intl.DateTimeFormat("en-US",{weekday:"long",month:"long",day:"numeric",year:"numeric",hour:event.all_day?undefined:"numeric",minute:event.all_day?undefined:"2-digit",timeZone:"Asia/Tokyo"}).format(start);
