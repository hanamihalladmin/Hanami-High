...
export default function LiveNextEvent(){
 const [event,setEvent]=useState<EventRow>(fallback);const [live,setLive]=useState(false);const [nowMs,setNowMs]=useState<number|null>(null);
 useEffect(()=>{let cancelled=false;setNowMs(Date.now());const clock=window.setInterval(()=>setNowMs(Date.now()),60000);async function load(){try{const now=encodeURIComponent(new Date().toISOString());const response=await fetch(`${SUPABASE_URL}/rest/v1/school_calendar_events?select=id,title,description,location,starts_at,all_day,category,featured,is_test_data&starts_at=gte.${now}&order=featured.desc,starts_at.asc&limit=1`,{headers:{apikey:SUPABASE_PUBLISHABLE_KEY}});if(!response.ok)return;const rows=await response.json() as EventRow[];if(!cancelled&&rows[0]){setEvent(rows[0]);setLive(true);}}catch{/* Retain labeled fallback. */}}load();return()=>{cancelled=true;window.clearInterval(clock);};},[]);
