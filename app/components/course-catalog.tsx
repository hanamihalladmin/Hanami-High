...
export default function CourseCatalog(){
  ...
  return <section className="info-section course-section" id="courses"><div className="section-heading"><h2>COURSE CATALOG</h2><span>TEST CATALOG • {matches.length} SHOWN</span></div><div className="test-data-notice"><strong>DEVELOPMENT DATA</strong><p>These sample classes are being used to test the website. They are not official roleplay enrollments or academic records.</p></div><div className="course-controls"><label><span>Search courses</span><input type="search" value={query} onChange={event=>setQuery(event.target.value)} placeholder="Search title, code, or topic"/></label><label><span>Department</span><select value={department} onChange={event=>setDepartment(event.target.value)}><option>All</option><option>Humanities</option><option>Mathematics</option><option>Sciences</option><option>Arts & Design</option><option>Pathways</option></select></label><label><span>Level</span><select value={level} onChange={event=>setLevel(event.target.value)}><option>All</option><option>Foundation</option><option>Intermediate</option><option>Honors</option></select></label></div><div className="course-results" aria-live="polite">{matches.length?matches.map(course=><article key={course.code}><div className="course-code"><strong>{course.code}</strong><span>{course.credits} CR</span></div><div><p className="eyebrow">{course.department} • {course.level}</p><h3>{course.title}</h3><p>{course.description}</p></div></article>):<p className="no-results">No courses match those filters.</p>}</div><p className="content-note">This public guide is a planning overview. Enrollment availability, prerequisites, and teacher assignments will appear in the student portal.</p></section>;
}
