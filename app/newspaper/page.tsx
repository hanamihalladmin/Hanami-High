...
export default function NewspaperPage(){
 ...
 useEffect(()=>{let dead=false;(async()=>{try{const r=await fetch(`${U}/rest/v1/rpc/public_newspaper_feed`,{method:"POST",headers:{apikey:K,"Content-Type":"application/json"},body:JSON.stringify({row_limit:80}),cache:"no-store"});if(!r.ok)throw new Error("The newspaper could not be loaded.");const data=await r.json() as Article[];if(!dead){setArticles(data);setStatus(data.length?"The latest Hanami stories are live.":"No issue has been published yet.")}}catch(e){if(!dead)setStatus(e instanceof Error?e.message:"The newspaper could not be loaded.")}})();return()=>{dead=true}},[]);
