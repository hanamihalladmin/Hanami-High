...
export default function HomeDashboard({currentDate}:Props){
 ...
 return <main id="home" className={styles.shell}>
  <aside className={styles.rail} aria-label="Public website navigation">
   ...
   <div className={styles.railLinks}><a href="./about/"><span>◎</span><b>About</b></a><a href="./rules/"><span>◆</span><b>Rules</b></a><a href="./apply/"><span>＋</span><b>Apply</b></a></div>
   <div className={styles.railBottom}><a href="./portal/"><span className={styles.railIcon}>↪</span><b>Portal Login</b></a></div>
  </aside>
  <section className={styles.main}>
   <header className={styles.topbar}><div><p>PUBLIC SCHOOL NETWORK • EST. 1836</p><h1>HANAMI HIGH SCHOOL</h1></div><div className={styles.dateCard}><strong>{currentDate.toUpperCase()}</strong><span>HANAMI CITY • JAPAN STANDARD TIME</span><LiveSchoolStatus/></div></header>
   <div className={styles.content}><LiveAnnouncements/>
    {tab==="home"&&<div className={styles.tabView}><section className={styles.hero}><div className={styles.heroMain}><p className={styles.eyebrow}>HANAMI HIGH • SCHOOL HOME</p><h2>A school day is more than a schedule.</h2><p>Explore academics, student life, clubs, school traditions, and the stories shaping Hanami from one compact school dashboard.</p><div className={styles.actions}><button type="button" onClick={()=>setTab("academics")}>Explore academics</button><a href="./apply/">Apply to Hanami</a><a href="./portal/">Open portal</a></div></div><aside className={`${styles.card} ${styles.todayCard}`}><div className={styles.cardHeader}><h3>TODAY AT HANAMI</h3><span>LIVE</span></div><p><b>{currentDate.toUpperCase()}</b></p><LiveNextEvent/></aside></section><section className={styles.quickRow}><button type="button" onClick={()=>setTab("academics")}><b>Academics</b><span>Courses and pathways →</span></button><button type="button" onClick={()=>setTab("campus")}><b>Campus Life</b><span>Clubs and opportunities →</span></button><button type="button" onClick={()=>setTab("events")}><b>Calendar</b><span>Upcoming school events →</span></button><a href="./portal/"><b>Portal</b><span>Student, Faculty, Admin & Owner →</span></a></section></div>}
    ...
    {tab==="campus"&&<section className={styles.focusCard}><div className={styles.cardHeader}><h3>CAMPUS LIFE</h3><a href="./campus-life/">OPEN CAMPUS LIFE →</a></div><div className={styles.liveWrap}><div><h3>Clubs & organizations</h3><p>Sports, academic, arts, service, and special-interest groups.</p><div className={styles.actions}><a href="./campus-life/#directory">View clubs</a></div></div><div><h3>Campus opportunities</h3><p>Jobs, volunteering, internships, and student leadership opportunities.</p><div className={styles.actions}><a href="./campus-life/#jobs">See opportunities</a></div></div></div></section>}
