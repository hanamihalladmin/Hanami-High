...
export default function StudentSettingsDrawer({accessToken,characterId}:Props){
 const [open,setOpen]=useState(false),[skin,setSkin]=useState<Skin>({}),[fontFamily,setFontFamily]=useState("default"),[websiteBackground,setWebsiteBackground]=useState("#fffafa"),[notice,setNotice]=useState("");
 ...
 return <><button type="button" className={styles.settingsButton} onClick={()=>setOpen(true)} aria-label="Open student settings"><span aria-hidden="true">⚙</span><b>Settings</b></button>{open&&<div className={styles.backdrop} role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)setOpen(false)}}><section className={styles.drawer} role="dialog" aria-modal="true" aria-labelledby="student-settings-title"><header className={styles.header}><div><small>STUDENT SETTINGS</small><h2 id="student-settings-title">Settings</h2><p>Personalize how the Hanami website looks and behaves for you.</p></div><button type="button" onClick={()=>setOpen(false)} aria-label="Close settings">×</button></header><div className={styles.content}><section className={styles.visualSection}><div className={styles.sectionHeading}><div><h3>Website appearance</h3><p>These settings change the portal website itself, not your dashboard wallpaper or course-card designs.</p></div><button type="button" onClick={resetVisuals}>Reset appearance</button></div><div className={styles.controlGrid}><label>Website font<select value={fontFamily} onChange={e=>{setFontFamily(e.target.value);apply(e.target.value,websiteBackground);}} onBlur={()=>void save()}>{FONT_OPTIONS.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><label>Website background color<div className={styles.colorRow}><input type="color" value={websiteBackground} onChange={e=>{setWebsiteBackground(e.target.value);apply(fontFamily,e.target.value);}} onBlur={()=>void save()}/><input type="text" value={websiteBackground} onChange={e=>setWebsiteBackground(e.target.value)} onBlur={()=>void save()} aria-label="Website background hex color"/></div></label></div><div className={styles.preview} style={{fontFamily:fontFamily==="default"?undefined:fontFamily,background:websiteBackground}}><strong>Preview</strong><span>Hanami High • Student Portal • Assignments • Messages</span></div>{notice&&<p className={styles.notice}>{notice}</p>}</section><NotificationAccessibilityPanel accessToken={accessToken} characterId={characterId}/></div></section></div>}</>;
}
