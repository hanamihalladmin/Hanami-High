...
import StudentBulletinSocialFeed from "./StudentBulletinSocialFeed";
import FacultyPetalAwardPanel from "./FacultyPetalAwardPanel";
...
export default function PortalContextBar({accessToken,view,subView,role}:{accessToken:string;view:string;subView:string;role:Role}){
 ...
 useEffect(()=>{let dead=false;(async()=>{try{const userR=await fetch(`${U}/auth/v1/user`,{headers:h(accessToken),cache:"no-store"});if(!userR.ok)return;const user=await userR.json() as {id?:string};if(!user.id||dead)return;const characterR=await fetch(`${U}/rest/v1/characters?select=id,display_name&owner_user_id=eq.${encodeURIComponent(user.id)}&is_active=eq.true&role=eq.${role}&limit=1`,{headers:h(accessToken),cache:"no-store"});const rows=characterR.ok?await characterR.json() as {id:string;display_name:string}[]:[];if(!dead){setCharacterId(rows[0]?.id??"");setCharacterName(rows[0]?.display_name??"")}}catch{if(!dead){setCharacterId("");setCharacterName("")}}})();return()=>{dead=true}},[accessToken,role]);
 ...
 return <><aside className={styles.bar} aria-label="Page sections and portal utilities"><div className={styles.topline}><div className={styles.greeting}><strong>{schoolGreeting()}{characterName?`, ${characterName}`:""}!</strong><span>{role==="student"?"Your Hanami school desk is ready.":"Your Hanami teaching desk is ready."}</span></div><div className={styles.utility}><button type="button" onClick={toggleDensity}>Density: {density==="comfortable"?"Comfortable":"Compact"}</button>{change&&<span title={change.summary}>Updated{change.version?` · v${change.version}`:""}</span>}</div></div><nav className={styles.tabs} aria-label="Page sections">{tabs.map(item=><button type="button" key={`${item.view}.${item.subView}`} className={item.view===view&&item.subView===subView?styles.active:""} aria-current={item.view===view&&item.subView===subView?"page":undefined} onClick={()=>go(item)}>{item.label}</button>)}</nav></aside>{role==="faculty"&&view==="school"&&subView==="tools"&&characterId&&<FacultyPetalAwardPanel accessToken={accessToken}/>} {role==="student"&&view==="community"&&subView==="boards"&&characterId&&<StudentBulletinSocialFeed accessToken={accessToken} characterId={characterId}/>} {view==="profile"&&subView==="appearance"&&characterId&&<AppearanceCollectiblesPanel accessToken={accessToken} characterId={characterId}/>} {view==="profile"&&subView==="rewards"&&characterId&&<RewardsCustomizationPanel accessToken={accessToken} characterId={characterId} mode="rewards"/>}{view==="profile"&&subView==="exchange"&&characterId&&<RewardsCustomizationPanel accessToken={accessToken} characterId={characterId} mode="exchange"/>}</>;
}
