...
export default function PortalContextBar({accessToken,view,subView,role}:{accessToken:string;view:string;subView:string;role:Role}){
 ...
 return <><aside className={styles.bar} aria-label="Page sections and portal utilities"><div className={styles.topline}><div className={styles.greeting}><strong>{schoolGreeting()}{characterName?`, ${characterName}`:""}!</strong><span>{role==="student"?"Your Hanami school desk is ready.":"Your Hanami teaching desk is ready."}</span></div><div className={styles.utility}><button type="button" onClick={toggleDensity}>Density: {density==="comfortable"?"Comfortable":"Compact"}</button>{change&&<span title={change.summary}>Updated{change.version?` · v${change.version}`:""}</span>}</div></div><nav className={styles.tabs} aria-label="Page sections">{tabs.map(item=><button type="button" key={`${item.view}.${item.subView}`} className={item.view===view&&item.subView===subView?styles.active:""} aria-current={item.view===view&&item.subView===subView?"page":undefined} onClick={()=>go(item)}>{item.label}</button>)}</nav></aside>{role==="faculty"&&view==="school"&&subView==="tools"&&<FacultyPetalAwardPanel accessToken={accessToken}/>} {role==="student"&&view==="community"&&subView==="boards"&&characterId&&<StudentBulletinSocialFeed accessToken={accessToken} characterId={characterId}/>} {view==="profile"&&subView==="appearance"&&characterId&&<AppearanceCollectiblesPanel accessToken={accessToken} characterId={characterId}/>} {view==="profile"&&subView==="rewards"&&characterId&&<RewardsCustomizationPanel accessToken={accessToken} characterId={characterId} mode="rewards"/>}{view==="profile"&&subView==="exchange"&&characterId&&<RewardsCustomizationPanel accessToken={accessToken} characterId={characterId} mode="exchange"/>}</>;
}
