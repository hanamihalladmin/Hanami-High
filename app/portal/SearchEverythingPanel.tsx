...
export default function SearchEverythingPanel({accessToken,characterId}:Props){
 ...
 function openProfile(handle:string){window.dispatchEvent(new CustomEvent("hanami-open-profile",{detail:{handle,viewerCharacterId:characterId}}));const target=document.getElementById("dashboard-profile-lookup");if(target instanceof HTMLDetailsElement)target.open=true;target?.scrollIntoView({behavior:"smooth",block:"start"});}
