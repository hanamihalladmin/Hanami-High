...
export default function OwnerControlCenter({accessToken}:Props){
 ...
 useEffect(()=>{let dead=false;(async()=>{try{
  ...
  if(!dead){setCharacters(directory.characters??[]);setFlags(flagRows);setSanctions(sanctionRows);setRuntime(runtimeRows);setMessage("Owner network state is current across all visible portals.");}
 ...
 const activeCharacters=useMemo(()=>characters.filter(x=>x.is_active!==false),[characters]);
 const students=useMemo(()=>activeCharacters.filter(x=>x.role==="student").length,[activeCharacters]);
 const faculty=useMemo(()=>activeCharacters.filter(x=>x.role==="faculty").length,[activeCharacters]);
 ...
 return <><section className={styles.panel}><div className={styles.heading}><div><p className="eyebrow">OWNER CONTROL CENTER</p><h3>Hanami network authority</h3></div><span>{maintenance?"MAINTENANCE":"NETWORK LIVE"}</span></div><div className={styles.stats}><article><span>STUDENTS</span><strong>{students}</strong></article><article><span>FACULTY</span><strong>{faculty}</strong></article><article><span>FEATURE FLAGS ON</span><strong>{enabledFlags}/{flags.length}</strong></article><article><span>ACTIVE SANCTIONS</span><strong>{sanctions.length}</strong></article><article><span>RP CLOCK</span><strong className={styles.mode}>{clockMode}</strong></article></div><div className={styles.matrix}><div><strong>Owner</strong><span>All portal previews</span><span>All Administration tools</span><span>Network controls</span><span>Clock & maintenance</span></div><div><strong>Administrator</strong><span>School operations</span><span>People & academics</span><span>Content/moderation</span><span>No Owner controls</span></div><div><strong>Faculty</strong><span>Teaching sections</span><span>Attendance & grading</span><span>Student advising</span><span>No admin controls</span></div><div><strong>Student</strong><span>Own courses</span><span>Own records</span><span>Activities/community</span><span>No staff controls</span></div></div><p className={styles.status}>{message}</p></section><OwnerPortalExpansionPanel/></>;
}
