...
export default function OwnerHomeroomScheduleEditor({accessToken}:{accessToken:string}){
 ...
 function addRow(){const previous=rows[rows.length-1],startsAt=previous?.endsAt??"08:00";setRows(current=>[...current,{id:makeId(),label:`Period ${current.length+1}`,startsAt,endsAt:addMinutes(startsAt,30),assignments:assignments()}]);}
