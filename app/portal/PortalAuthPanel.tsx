...
export default function PortalAuthPanel(){
  ...
  const finishSignedIn=useCallback(async(session:PortalSession)=>{
    saveSession(session);
    const account=await loadProfile(session.accessToken);
    setProfile(account);
    setState("signed-in");
  ...
  return <>
    ...
    {state==="signed-in"?<div className="portal-account-card"><p className="eyebrow">CURRENT HANAMI ACCOUNT</p><h2>{profile?.display_name??"Hanami Member"}</h2>{profile?.discord_username&&<p className="portal-handle">@{profile.discord_username}</p>}<p className="portal-account-note">Authentication only uses Discord for identity. School communication stays inside Hanami High.</p><button className="secondary-action portal-signout" type="button" onClick={signOut}>Sign out on this device</button></div>:<button className="discord-button" type="button" onClick={signIn} disabled={state==="checking"}><span>◉</span>{state==="checking"?" Checking session…":" Continue with Discord"}</button>}
