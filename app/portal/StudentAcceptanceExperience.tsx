...
export default function StudentAcceptanceExperience({accessToken,characterId,displayName}:Props){
 ...
 function Letter(){return <div className={styles.letterWrap}><article className={styles.letter}>
  ...
  <div className={styles.signature}><div className={styles.signatureText}><strong>Office of Admissions</strong><span>HANAMI HIGH SCHOOL · {dateLabel().toUpperCase()}</span></div><div className={styles.wax}>HH</div></div>
  <div className={styles.actions}><button type="button" onClick={markViewed}>Enter Hanami High →</button>{archived&&<button className={styles.secondary} type="button" onClick={()=>setState("closed")}>Close letter</button>}</div>
 </article></div>}
 ...
 return <div className={styles.overlay} role="dialog" aria-modal="true" aria-label="Hanami High acceptance letter">
  <section className={styles.stage}>
   {state==="envelope"?<div className={styles.envelope}><button className={styles.sealButton} type="button" onClick={()=>setState("letter")}><span className={styles.seal}>HH</span><strong>Hanami High School</strong><span>OPEN YOUR ACCEPTANCE LETTER</span></button></div>:<Letter/>}
  </section>
 </div>;
}
