...
export default function StudentAcademicRecordPanel({accessToken,characterId}:Props){
 ...
 const attendanceCounts=attendance.reduce<Record<string,number>>((acc,row)=>({...acc,[row.status]:(acc[row.status]??0)+1}),{});const homeroom=homeroomRecord?.homerooms??null;const year=homeroomRecord?.student_year??null;const gradeLabel=year?`${year}${year===1?"st":year===2?"nd":"th"} Year`:"Pending";
 ...
 <section className={styles.idSection} aria-labelledby="student-id-title"><div className={styles.idIntro}><div><p className="eyebrow">DIGITAL STUDENT IDENTIFICATION</p><h5 id="student-id-title">Hanami High Student ID</h5></div><span>{idStatus}</span></div><div className={styles.idCard}><div className={styles.idTop}><div><strong>花見高等学校</strong><span>HANAMI HIGH SCHOOL</span></div><b>STUDENT</b></div><div className={styles.idBody}><div className={styles.idPhoto}>{avatarUrl?<img src={avatarUrl} alt={`${student?.display_name??"Student"} profile`}/>:<span>{student?.display_name?.slice(0,1).toUpperCase()??"花"}</span>}</div><div className={styles.idDetails}><h6>{student?.display_name??"Student"}</h6><p>@{student?.handle??"student"}</p><dl><div><dt>ID Number</dt><dd>{idCard?.id_number??"Issuing…"}</dd></div><div><dt>Homeroom</dt><dd>{homeroom?`${homeroom.code}${homeroom.room_label?` • ${homeroom.room_label}`:""}`:"Assigned with schedule"}</dd></div><div><dt>Year</dt><dd>{gradeLabel}</dd></div><div><dt>School Year</dt><dd>{idCard?.school_year??homeroom?.school_year??"2006-2007"}</dd></div><div><dt>Status</dt><dd>{idCard?.status?.toUpperCase()??"PENDING"}</dd></div><div><dt>Issued</dt><dd>{idCard?.issued_at?new Intl.DateTimeFormat("en-US",{timeZone:"Asia/Tokyo",month:"short",day:"numeric",year:"numeric"}).format(new Date(idCard.issued_at)):"—"}</dd></div></dl></div></div><div className={styles.idFooter}><span>HANAMI CITY • JAPAN</span><span>PROPERTY OF HANAMI HIGH SCHOOL</span></div></div><p className={styles.idNote}>Your student picture follows your portal profile image automatically. Homeroom and student year are separate: A/B/C homerooms may include both 1st-year and 2nd-year students. Your school year remains 2006-2007 unless the Owner or Administration changes the ID record.</p></section>
