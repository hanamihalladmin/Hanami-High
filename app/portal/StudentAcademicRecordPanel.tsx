...
type IdCardRow={id_number:string;issued_at:string;status:string};
type HomeroomRow={joined_at:string;homerooms:{code:string;grade_level:number;school_year:string;room_label:string|null}|null};
...
export default function StudentAcademicRecordPanel({accessToken,characterId}:Props){
 ...
 const [student,setStudent]=useState<CharacterRow|null>(null);const [idCard,setIdCard]=useState<IdCardRow|null>(null);const [homeroom,setHomeroom]=useState<HomeroomRow["homerooms"]>(null);const [avatarUrl,setAvatarUrl]=useState("");const [idStatus,setIdStatus]=useState("Preparing student ID…");
 useEffect(()=>{let cancelled=false;let objectUrl="";(async()=>{try{
   ...
   const [characterResponse,idResponse,homeroomResponse,preferenceResponse]=await Promise.all([
    ...
    fetch(`${SUPABASE_URL}/rest/v1/student_id_cards?select=id_number,issued_at,status&character_id=eq.${encodeURIComponent(characterId)}&limit=1`,{headers:headers(accessToken)}),
    fetch(`${SUPABASE_URL}/rest/v1/homeroom_memberships?select=joined_at,homerooms(code,grade_level,school_year,room_label)&student_character_id=eq.${encodeURIComponent(characterId)}&limit=1`,{headers:headers(accessToken)}),
   ...
   const character=(await characterResponse.json() as CharacterRow[])[0]??null;const card=(await idResponse.json() as IdCardRow[])[0]??null;const room=(await homeroomResponse.json() as HomeroomRow[])[0]?.homerooms??null;const preference=(await preferenceResponse.json() as PortalPreferenceRow[])[0]??null;
   ...
   if(cancelled){if(objectUrl)URL.revokeObjectURL(objectUrl);return;}setStudent(character);setIdCard(card);setHomeroom(room);setAvatarUrl(objectUrl);setIdStatus(card?"Official Hanami student ID ready.":"Student ID unavailable.");
 ...
 const attendanceCounts=attendance.reduce<Record<string,number>>((acc,row)=>({...acc,[row.status]:(acc[row.status]??0)+1}),{});const gradeLabel=homeroom?.grade_level?`${homeroom.grade_level}${homeroom.grade_level===1?"st":homeroom.grade_level===2?"nd":homeroom.grade_level===3?"rd":"th"} Year`:"Pending";
 ...
 <section className={styles.idSection} aria-labelledby="student-id-title"><div className={styles.idIntro}><div><p className="eyebrow">DIGITAL STUDENT IDENTIFICATION</p><h5 id="student-id-title">Hanami High Student ID</h5></div><span>{idStatus}</span></div><div className={styles.idCard}><div className={styles.idTop}><div><strong>花見高等学校</strong><span>HANAMI HIGH SCHOOL</span></div><b>STUDENT</b></div><div className={styles.idBody}><div className={styles.idPhoto}>{avatarUrl?<img src={avatarUrl} alt={`${student?.display_name??"Student"} profile`}/>:<span>{student?.display_name?.slice(0,1).toUpperCase()??"花"}</span>}</div><div className={styles.idDetails}><h6>{student?.display_name??"Student"}</h6><p>@{student?.handle??"student"}</p><dl><div><dt>ID Number</dt><dd>{idCard?.id_number??"Issuing…"}</dd></div><div><dt>Homeroom</dt><dd>{homeroom?`${homeroom.code}${homeroom.room_label?` • ${homeroom.room_label}`:""}`:"Assigned with schedule"}</dd></div><div><dt>Year</dt><dd>{gradeLabel}</dd></div><div><dt>School Year</dt><dd>{homeroom?.school_year??"Pending schedule"}</dd></div><div><dt>Status</dt><dd>{idCard?.status?.toUpperCase()??"PENDING"}</dd></div><div><dt>Issued</dt><dd>{idCard?.issued_at?new Intl.DateTimeFormat("en-US",{timeZone:"Asia/Tokyo",month:"short",day:"numeric",year:"numeric"}).format(new Date(idCard.issued_at)):"—"}</dd></div></dl></div></div><div className={styles.idFooter}><span>HANAMI CITY • JAPAN</span><span>PROPERTY OF HANAMI HIGH SCHOOL</span></div></div><p className={styles.idNote}>Your student picture follows your portal profile image automatically. Homeroom, year, and school year update when your official schedule assignment is added. Your generated ID number stays attached to this student character.</p></section>
