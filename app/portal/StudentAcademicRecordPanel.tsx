...
export default function StudentAcademicRecordPanel({accessToken,characterId}:Props){
 ...
 <section className={styles.idSection} aria-labelledby="student-id-title"><div className={styles.idIntro}><div><p className="eyebrow">DIGITAL STUDENT IDENTIFICATION</p><h5 id="student-id-title">Hanami High Student ID</h5></div><span>{idStatus}</span></div><div className={styles.idCard}><div className={styles.idTop}><div><strong>HANAMI HIGH</strong><span>HANAMI HIGH SCHOOL</span></div><b>STUDENT</b></div><div className={styles.idBody}><div className={styles.idPhoto}>{avatarUrl?<img src={avatarUrl} alt={`${student?.display_name??"Student"} profile`}/>:<span>{student?.display_name?.slice(0,1).toUpperCase()??"H"}</span>}</div><div className={styles.idDetails}><h6>{student?.display_name??"Student"}</h6><p>@{student?.handle??"student"}</p><dl><div><dt>ID Number</dt><dd>{idCard?.id_number??"Issuing…"}</dd></div><div><dt>Homeroom</dt><dd>{homeroom?`${homeroom.code}${homeroom.room_label?` • ${homeroom.room_label}`:""}`:"Assigned with schedule"}</dd></div><div><dt>Year</dt><dd>{gradeLabel}</dd></div><div><dt>School Year</dt><dd>{idCard?.school_year??homeroom?.school_year??"2006-2007"}</dd></div><div><dt>Status</dt><dd>{idCard?.status?.toUpperCase()??"PENDING"}</dd></div><div><dt>Issued</dt><dd>Apr 7, 2006</dd></div></dl></div></div><div className={styles.idFooter}><span>HANAMI CITY</span><span>PROPERTY OF HANAMI HIGH SCHOOL</span></div></div><p className={styles.idNote}>All Hanami High student IDs are institutionally issued on April 7, 2006, the school opening date. Your student picture follows your portal profile image automatically. Homeroom and student year are separate: A/B/C homerooms may include both 1st-year and 2nd-year students.</p></section>
