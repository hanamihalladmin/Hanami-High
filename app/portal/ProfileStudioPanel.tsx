...
export default function ProfileStudioPanel({accessToken,characterId}:Props){
 ...
 async function patchWidget(id:string,patch:Partial<Widget>,track=true){
  ...
  setWidgets(current=>current.map(widget=>{if(widget.id!==id)return widget;next={...widget,...patch};return next;}));
 ...
 function startDrag(event:PointerEvent<HTMLDivElement>,widget:Widget){
  ...
  const scale=canvas?Math.min(1,(event.currentTarget.parentElement?.clientWidth??canvas.canvas_width)/canvas.canvas_width):1;
  drag.current={startX:event.clientX,startY:event.clientY,scale,bases};
 }
 ...
 function startResize(event:PointerEvent<HTMLButtonElement>,widget:Widget){
  event.stopPropagation();if(widget.locked||!canvas)return;event.currentTarget.setPointerCapture(event.pointerId);checkpoint();const scale=Math.min(1,(event.currentTarget.closest(`.${styles.canvas}`)?.clientWidth??canvas.canvas_width)/canvas.canvas_width);resize.current={id:widget.id,startX:event.clientX,startY:event.clientY,scale,width:widget.width,height:widget.height};
 }
 ...
 function renderWidget(widget:Widget){
  ...
  return <div style={common}>{widget.content.text||widget.widget_type.toUpperCase()}</div>;
 }
 ...
 return <section className={styles.studio} aria-labelledby="profile-studio-title">
  ...
  <div className={styles.toolbar}>
   {(["text","image","card","link","divider","sticker"] as WidgetType[]).map(type=><button key={type} type="button" onClick={()=>addWidget(type)} disabled={saving}>+ {type}</button>)}
  ...
  <div className={styles.workspace}>
   ...
   <aside className={styles.inspector}>
    ...
    {selected&&<>
     <h5>Selected {selected.widget_type}</h5>
     {selected.widget_type!=="divider"&&selected.widget_type!=="image"&&<label><span>Text</span><textarea value={selected.content.text??""} onChange={event=>updateContent("text",event.target.value)}/></label>}
     {selected.widget_type==="image"&&<><label><span>Image URL</span><input value={selected.content.url??""} onChange={event=>updateContent("url",event.target.value)}/></label><label><span>Alt text</span><input value={selected.content.alt??""} onChange={event=>updateContent("alt",event.target.value)}/></label></>}
  ...
  <div className={styles.note}><strong>DESIGN FREEDOM</strong><span>Profiles are built from independent widgets instead of fixed templates. Multi-select, resize handles, alignment tools, undo/redo, templates, layering, locking, and freeform styling are now part of the editor foundation.</span></div>
 </section>;
}
