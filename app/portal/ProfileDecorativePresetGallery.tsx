...
type Preset={name:string;category:string;widget_type:WidgetType;width:number;height:number;content:Record<string,string>;style:Record<string,string|number>};
...
const presets:Preset[]=[
 ...
 {name:"Dotted Notebook",category:"Dividers",widget_type:"divider",width:460,height:10,content:{},style:{background:"transparent",borderRadius:0,borderWidth:3,borderColor:"#7690aa",borderStyle:"dotted",boxShadow:"none"}},
 {name:"Soft Glow",category:"Dividers",widget_type:"divider",width:460,height:10,content:{},style:{background:"#f3bfd4",borderRadius:99,borderWidth:0,borderColor:"#f3bfd4",borderStyle:"solid",boxShadow:"0 0 16px rgba(180,73,114,.35)"}},
 ...
 {name:"Music Note",category:"Stickers",widget_type:"sticker",width:100,height:100,content:{text:"♫"},style:{fontSize:58,fontFamily:"Georgia, serif",color:"#17375f",background:"#eaf1f7",borderRadius:18,textAlign:"center",borderWidth:1,borderColor:"#adc0d3",borderStyle:"solid",boxShadow:"4px 4px 0 rgba(23,55,95,.20)"}},
 {name:"Lucky Star",category:"Stickers",widget_type:"sticker",width:108,height:108,content:{text:"★"},style:{fontSize:62,fontFamily:"Georgia, serif",color:"#ffffff",background:"#17375f",borderRadius:22,textAlign:"center",borderWidth:3,borderColor:"#d7a7bb",borderStyle:"solid",boxShadow:"none"}},
 {name:"Cute Bow",category:"Stickers",widget_type:"sticker",width:120,height:90,content:{text:"🎀"},style:{fontSize:52,fontFamily:"Arial, sans-serif",color:"#8e4364",background:"#fff7fb",borderRadius:20,textAlign:"center",borderWidth:1,borderColor:"#ead0dc",borderStyle:"solid",boxShadow:"none"}},
 {name:"Cloud",category:"Stickers",widget_type:"sticker",width:130,height:86,content:{text:"☁"},style:{fontSize:64,fontFamily:"Arial, sans-serif",color:"#7891aa",background:"#f6fbff",borderRadius:99,textAlign:"center",borderWidth:1,borderColor:"#c5d6e5",borderStyle:"solid",boxShadow:"0 3px 8px rgba(23,55,95,.10)"}},

 {name:"Student Club",category:"Badges",widget_type:"badge",width:190,height:66,content:{text:"HANAMI CLUB"},style:{fontSize:15,fontFamily:"Arial, sans-serif",color:"#ffffff",background:"#8e4364",borderRadius:99,textAlign:"center",borderWidth:0,borderColor:"#8e4364",borderStyle:"solid",boxShadow:"none"}},
...
export default function ProfileDecorativePresetGallery({accessToken,characterId,onAdded}:Props){
 const [category,setCategory]=useState("Dividers");
 const [busy,setBusy]=useState("");
 const [notice,setNotice]=useState("");
 const categories=["Dividers","Stickers","Badges","Cards"];
 async function addPreset(preset:Preset){if(busy)return;setBusy(preset.name);setNotice(`Adding ${preset.name}…`);try{
 ...
 }catch(error){setNotice(error instanceof Error?error.message:"Decorative preset could not be added.");}finally{setBusy("");}}
 return <section style={{marginTop:12,border:"1px solid #c4b2bd",background:"#fff",overflow:"hidden"}} aria-label="Decorative element preset library">
   ...
   <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:8,padding:12}}>{presets.filter(p=>p.category===category).map(preset=><button key={preset.name} type="button" disabled={Boolean(busy)} onClick={()=>addPreset(preset)} style={{minHeight:88,padding:10,border:"1px solid #c5ced8",background:"#fff",color:"#17375f",textAlign:"left",cursor:"pointer",opacity:busy&&busy!==preset.name?.7:1}}><span style={{display:"block",fontSize:9,fontWeight:700,marginBottom:5}}>{preset.name}</span><span style={{display:"flex",alignItems:"center",justifyContent:"center",height:42,border:"1px solid #e0e5ea",background:String(preset.style.background??"#fff"),color:String(preset.style.color??preset.style.borderColor??"#17375f"),borderRadius:Number(preset.style.borderRadius??0),fontSize:preset.widget_type==="sticker"?28:10,overflow:"hidden"}}>{preset.widget_type==="divider"?"━━━━":preset.content.text??preset.name}</span></button>)}</div>
