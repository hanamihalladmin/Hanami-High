...
type Tool="overview"|"status"|"announcements"|"academics"|"homerooms"|"schedule"|"deletion"|"operations"|"exams"|"directory"|"ids"|"passes"|"narrative"|"moderation"|"events"|"opportunities"|"approvals"|"roadmap"|"lore"|"requests"|"tickets"|"roleplay"|"continuity"|"roadmapOps"|"transit"|"city"|"community";
 ...
 return <section className={styles.shell}>
  ...
  <main className={styles.main}><header className={styles.header}><div><p>{ownerMode?"Owner":"Administration"} Portal</p><h1>{nav.find(item=>item[0]===view)?.[2]}</h1></div><span className={styles.badge}>{ownerMode?"OWNER":"ADMIN"}</span></header>
   ...
   <section className={styles.content}>
    ...
    {view==="classes"&&tool==="academics"&&access.site_admin&&<AdminAcademicManager accessToken={accessToken}/>} {view==="classes"&&tool==="homerooms"&&access.site_admin&&<AdminHomeroomManager accessToken={accessToken}/>} {view==="classes"&&tool==="schedule"&&access.site_admin&&<AdminScheduleBlockManager accessToken={accessToken}/>} {view==="classes"&&tool==="deletion"&&access.site_admin&&<AdminDeletionManager accessToken={accessToken}/>} {view==="classes"&&tool==="operations"&&access.site_admin&&<AdminOperationsExpansionPanel accessToken={accessToken} userId={userId}/>} {view==="classes"&&tool==="exams"&&access.site_admin&&<AdminCompetitionExamManager accessToken={accessToken} userId={userId}/>} 
