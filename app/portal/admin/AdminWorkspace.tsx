...
import AdminWebsiteOverview from "./AdminWebsiteOverview";
...
import OwnerOperationsCommandPanel from "../owner/OwnerOperationsCommandPanel";
import OwnerPortalExpansionPanel from "../owner/OwnerPortalExpansionPanel";
import OwnerDiscordRoleSyncPanel from "../owner/OwnerDiscordRoleSyncPanel";
import OwnerSystemHealthAnalyticsPanel from "../owner/OwnerSystemHealthAnalyticsPanel";
import OwnerOperationsPanel from "../owner/OwnerOperationsPanel";
import OwnerRoleplayClockPanel from "../owner/OwnerRoleplayClockPanel";
import OwnerFeedbackInboxPanel from "../owner/OwnerFeedbackInboxPanel";
import OwnerBugDetectorPanel from "../owner/OwnerBugDetectorPanel";
import OwnerGovernanceExpansionPanel from "../owner/OwnerGovernanceExpansionPanel";

type Access={site_admin:boolean;content_editor:boolean;moderator:boolean};
type View="home"|"classes"|"people"|"content"|"support"|"systems"|"academics"|"communications"|"campus"|"events"|"moderation"|"website"|"reports"|"settings"|"command"|"users"|"characters"|"portals"|"schoolData"|"economy"|"integrations"|"system";
type Tool="overview"|"status"|"announcements"|"academics"|"homerooms"|"classroomControl"|"dayChanges"|"schedule"|"deletion"|"operations"|"exams"|"directory"|"studentOverview"|"ids"|"passes"|"narrative"|"moderation"|"publishing"|"events"|"clubs"|"campusOps"|"drafts"|"opportunities"|"applications"|"approvals"|"roadmap"|"lore"|"requests"|"tickets"|"roleplay"|"economy"|"continuity"|"roadmapOps"|"transit"|"city"|"community"|"governance"|"analytics"|"reportsOverview"|"websiteOverview"|"settingsOverview"|"ownerProvisioning"|"portalOverview"|"feedback"|"discordSync"|"systemHealth"|"ownerOps"|"ownerClock"|"bugDetector"|"ownerGovernance";
...
const ownerNav:NavItem[]=[
 ["command","⌂","Command Center"],["users","●","Users"],["characters","◉","Characters"],["portals","▣","Portals"],["schoolData","▦","School Data"],["website","◎","Website"],["moderation","!","Moderation"],["economy","◇","Economy"],["integrations","↔","Integrations"],["system","⚙","System"],["settings","≡","Settings"]
];
const ownerToolMap:Record<string,ToolItem[]>={
 command:[["overview","Network Overview"]],
 users:[["ownerProvisioning","Administrator Provisioning"],["directory","Directory"],["applications","Applications & PA"],["requests","Office Requests"]],
 characters:[["studentOverview","Character Overview"],["ids","School IDs"],["passes","Hall Passes"],["deletion","Removal Tools"]],
 portals:[["portalOverview","Portal Oversight"],["classroomControl","Classroom Control Center"]],
 schoolData:[["status","School Status"],["academics","Courses & Sections"],["homerooms","Homerooms"],["dayChanges","Day Changes"],["schedule","Schedule Blocks"],["operations","Academic Operations"],["exams","Exams"],["events","Events"],["clubs","Club Publishing"],["campusOps","Campus Operations"],["opportunities","Opportunities"]],
 website:[["publishing","School Publishing"],["drafts","Draft Workspace"],["announcements","Announcements"],["roadmap","Website Roadmap"],["lore","Lore & Canon"]],
 moderation:[["moderation","Moderation"],["narrative","Narrative Review"],["approvals","Content Approvals"],["tickets","Support Tickets"],["feedback","Feedback Inbox"]],
 economy:[["economy","Economy & Exchange"]],
 integrations:[["discordSync","Discord Role Sync"]],
 system:[["systemHealth","System Health"],["ownerOps","Owner Operations"],["bugDetector","Bug Detector"],["roleplay","Roleplay Systems"],["ownerClock","Roleplay Clock"],["continuity","Continuity Archive"],["roadmapOps","Roadmap Operations"],["community","Community Ops"],["transit","Transit"],["city","City Lifecycle"]],
 settings:[["governance","Governance"],["ownerGovernance","Owner Governance"]]
};
...
const adminToolMap:Record<string,ToolItem[]>={
 ...
 website:[["websiteOverview","Website Overview"],["publishing","School Publishing"],["roadmap","Website Roadmap"],["lore","Lore & Canon"]],
 ...
 settings:[["settingsOverview","Settings Overview"],["governance","Governance"],["economy","Economy & Exchange"],["roleplay","Roleplay Systems"]]
};

export default function AdminWorkspace({accessToken,userId,access,ownerMode=false,ownerOverview,actions}:Props){
 const initialView:View=ownerMode?"command":"home";const [view,setView]=useState<View>(initialView);const [tool,setTool]=useState<Tool>("overview");const [secondaryOpen,setSecondaryOpen]=useState(true);
 ...
 function canUseTool(candidate:Tool){
  ...
  if(candidate==="websiteOverview")return canEditContent;
  if(candidate==="settingsOverview")return access.site_admin||canEditContent||canModerate;
 ...
 return <section className={styles.shell}>
  ...
  <main className={`${styles.main} ${secondaryOpen?"":styles.secondaryCollapsed}`}><header className={styles.header}><div><p>{ownerMode?"Owner":"Administration"} Portal</p><h1>{nav.find(item=>item[0]===view)?.[2]??(ownerMode?"Owner":"Administration")}</h1></div><div className={styles.headerActions}><button type="button" className={styles.secondaryToggle} aria-expanded={secondaryOpen} onClick={()=>setSecondaryOpen(v=>!v)}>{secondaryOpen?"Hide tools":"Show tools"}</button><span className={styles.badge}>{ownerMode?"OWNER":"ADMIN"}</span></div></header>
   ...
   <section className={styles.content}>
    {tool==="overview"&&<><div className={styles.pageIntro}><h2>{ownerMode?"Owner Command Center":"Administration Dashboard"}</h2><p>{ownerMode?"Network authority, school state, and Owner-only controls without requiring a character OC.":"People, academics, communications, campus, events, moderation, website operations, reports, and settings in one administration workspace."}</p></div><AdminCommandCenter accessToken={accessToken} access={access}/>{ownerMode&&<><OwnerControlCenter accessToken={accessToken}/><OwnerOperationsCommandPanel accessToken={accessToken}/></>}</>}
    {tool==="ownerProvisioning"&&ownerMode&&ownerOverview}
    {tool==="portalOverview"&&ownerMode&&<OwnerPortalExpansionPanel/>}
    {tool==="feedback"&&ownerMode&&<OwnerFeedbackInboxPanel accessToken={accessToken}/>} {tool==="discordSync"&&ownerMode&&<OwnerDiscordRoleSyncPanel accessToken={accessToken}/>} {tool==="systemHealth"&&ownerMode&&<OwnerSystemHealthAnalyticsPanel accessToken={accessToken}/>} {tool==="ownerOps"&&ownerMode&&<OwnerOperationsPanel accessToken={accessToken}/>} {tool==="ownerClock"&&ownerMode&&<OwnerRoleplayClockPanel accessToken={accessToken}/>} {tool==="bugDetector"&&ownerMode&&<OwnerBugDetectorPanel accessToken={accessToken}/>} {tool==="ownerGovernance"&&ownerMode&&<OwnerGovernanceExpansionPanel accessToken={accessToken} userId={userId}/>} 
    ...
    {tool==="classroomControl"&&ownerMode&&<OwnerClassroomControlCenter accessToken={accessToken} userId={userId}/>} {tool==="academics"&&access.site_admin&&<AdminAcademicManager accessToken={accessToken}/>} {tool==="homerooms"&&access.site_admin&&<AdminHomeroomManager accessToken={accessToken}/>} {tool==="dayChanges"&&access.site_admin&&<AdminClassDayChangesPanel accessToken={accessToken}/>} {tool==="schedule"&&access.site_admin&&<AdminScheduleBlockManager accessToken={accessToken}/>} {tool==="deletion"&&access.site_admin&&<AdminDeletionManager accessToken={accessToken}/>} {tool==="operations"&&access.site_admin&&<AdminOperationsExpansionPanel accessToken={accessToken} userId={userId}/>} {tool==="exams"&&access.site_admin&&<AdminCompetitionExamManager accessToken={accessToken} userId={userId}/>} 
    ...
    {tool==="websiteOverview"&&!ownerMode&&<AdminWebsiteOverview canEditContent={canEditContent}/>} {tool==="reportsOverview"&&!ownerMode&&<AdminReportsOverview siteAdmin={access.site_admin} canEditContent={canEditContent}/>} {tool==="settingsOverview"&&!ownerMode&&<AdminSettingsOverview siteAdmin={access.site_admin} canEditContent={canEditContent} canModerate={canModerate}/>} {tool==="governance"&&access.site_admin&&<AdminGovernancePanel accessToken={accessToken} ownerMode={ownerMode}/>} {tool==="analytics"&&access.site_admin&&<AdminStaffAnalyticsPanel accessToken={accessToken}/>} 
   </section>
  </main>
 </section>;
}
