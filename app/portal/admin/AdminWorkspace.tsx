...
export default function AdminWorkspace({accessToken,userId,access,ownerMode=false,ownerOverview,actions}:Props){
 ...
 const nav:[View,string,string][]=[["home","⌂","Dashboard"],["classes","▣",ownerMode?"Classrooms":"Courses"],["people","●","People"],["content","◇","Content"],["support","?","Support"],["systems","⚙",ownerMode?"System":"Admin"]];
 const toolMap:Record<View,[Tool,string][]>= {home:[["overview","Overview"],["status","School Status"],["announcements","Announcements"]],classes:[["academics","Courses & Sections"],["homerooms","Homerooms"],["dayChanges","Day Changes"],["schedule","Schedule Blocks"],["deletion","Removal Tools"],["operations","Academic Operations"],["exams","Exams"]],people:[["directory","Directory"],["studentOverview","Student Overview"],["ids","School IDs"],["passes","Hall Passes"],["narrative","Narrative Review"],["moderation","Moderation"]],content:[["publishing","School Publishing"],["events","Events"],["clubs","Club Publishing"],["campusOps","Campus Operations"],["drafts","Draft Workspace"],["opportunities","Opportunities"],["applications","Applications & PA"],["approvals","Approvals"],["roadmap","Roadmap"],["lore","Lore & Canon"]],support:[["requests","Office Requests"],["tickets","Support Tickets"]],systems:[["economy","Economy & Exchange"],["roleplay","Roleplay Systems"],["continuity","Continuity"],["roadmapOps","Roadmap Ops"],["transit","Transit"],["city","City Lifecycle"],["community","Community Ops"]]};
 if(ownerMode)toolMap.classes=[["classroomControl","Classroom Control Center"]];
 ...
 return <section className={styles.shell}>
  ...
  <main className={`${styles.main} ${secondaryOpen?"":styles.secondaryCollapsed}`}><header className={styles.header}><div><p>{ownerMode?"Owner":"Administration"} Portal</p><h1>{nav.find(item=>item[0]===view)?.[2]}</h1></div><div className={styles.headerActions}><button type="button" className={styles.secondaryToggle} aria-expanded={secondaryOpen} onClick={()=>setSecondaryOpen(v=>!v)}>{secondaryOpen?"Hide tools":"Show tools"}</button><span className={styles.badge}>{ownerMode?"OWNER":"ADMIN"}</span></div></header>
   <aside className={`${styles.secondaryNav} ${secondaryOpen?"":styles.secondaryNavCollapsed}`} aria-label={`${view} tools`}><strong>{secondaryOpen?nav.find(item=>item[0]===view)?.[2]:"Tools"}</strong>{secondaryOpen?<nav>{toolMap[view].map(([key,label])=><button key={key} type="button" className={tool===key?styles.toolActive:""} onClick={()=>setTool(key)}>{label}</button>)}</nav>:<button type="button" className={styles.reopenTools} aria-label="Show tool navigation" onClick={()=>setSecondaryOpen(true)}>›</button>}</aside>
   <section className={styles.content}>
    ...
    {view==="classes"&&tool==="classroomControl"&&ownerMode&&<OwnerClassroomControlCenter accessToken={accessToken} userId={userId}/>} {view==="classes"&&tool==="academics"&&!ownerMode&&access.site_admin&&<AdminAcademicManager accessToken={accessToken}/>} {view==="classes"&&tool==="homerooms"&&!ownerMode&&access.site_admin&&<AdminHomeroomManager accessToken={accessToken}/>} {view==="classes"&&tool==="dayChanges"&&!ownerMode&&access.site_admin&&<AdminClassDayChangesPanel accessToken={accessToken}/>} {view==="classes"&&tool==="schedule"&&!ownerMode&&access.site_admin&&<AdminScheduleBlockManager accessToken={accessToken}/>} {view==="classes"&&tool==="deletion"&&!ownerMode&&access.site_admin&&<AdminDeletionManager accessToken={accessToken}/>} {view==="classes"&&tool==="operations"&&!ownerMode&&access.site_admin&&<AdminOperationsExpansionPanel accessToken={accessToken} userId={userId}/>} {view==="classes"&&tool==="exams"&&!ownerMode&&access.site_admin&&<AdminCompetitionExamManager accessToken={accessToken} userId={userId}/>} 
    {view==="people"&&tool==="directory"&&hasAdminAccess&&<AdminCharacterDirectory accessToken={accessToken}/>} {view==="people"&&tool==="studentOverview"&&hasAdminAccess&&<AdminStudentOverviewPanel accessToken={accessToken}/>} {view==="people"&&tool==="ids"&&hasAdminAccess&&<AdminStudentIdManager accessToken={accessToken}/>} {view==="people"&&tool==="passes"&&hasAdminAccess&&<AdminHallPassManager accessToken={accessToken}/>} {view==="people"&&tool==="narrative"&&hasAdminAccess&&<AdminNarrativeModerationPanel accessToken={accessToken}/>} {view==="people"&&tool==="moderation"&&canModerate&&<AdminModerationManager accessToken={accessToken} userId={userId} access={access}/>} 
    {view==="content"&&tool==="publishing"&&canEditContent&&<AdminPublishingHub accessToken={accessToken}/>} {view==="content"&&tool==="events"&&canEditContent&&<AdminEventManager accessToken={accessToken} userId={userId} access={access}/>} {view==="content"&&tool==="clubs"&&canEditContent&&<AdminClubPublishingPanel accessToken={accessToken}/>} {view==="content"&&tool==="campusOps"&&access.site_admin&&<AdminCampusOperationsPanel accessToken={accessToken}/>} {view==="content"&&tool==="drafts"&&canEditContent&&<AdminDraftWorkspace accessToken={accessToken}/>} {view==="content"&&tool==="opportunities"&&canEditContent&&<AdminOpportunityManager accessToken={accessToken} userId={userId}/>} {view==="content"&&tool==="applications"&&access.site_admin&&<AdminApplicationReviewPanel accessToken={accessToken} userId={userId}/>} {view==="content"&&tool==="approvals"&&canEditContent&&<AdminContentApprovalPanel accessToken={accessToken}/>} {view==="content"&&tool==="roadmap"&&canEditContent&&<AdminRoadmapManager accessToken={accessToken} userId={userId}/>} {view==="content"&&tool==="lore"&&canEditContent&&<AdminLoreCanonEditor accessToken={accessToken}/>} 
