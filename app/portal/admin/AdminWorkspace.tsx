...
type Tool="overview"|"status"|"announcements"|"academics"|"homerooms"|"classroomControl"|"dayChanges"|"schedule"|"deletion"|"operations"|"exams"|"directory"|"studentOverview"|"ids"|"passes"|"narrative"|"moderation"|"events"|"campusOps"|"drafts"|"opportunities"|"applications"|"approvals"|"roadmap"|"lore"|"requests"|"tickets"|"roleplay"|"continuity"|"roadmapOps"|"transit"|"city"|"community";
...
export default function AdminWorkspace({accessToken,userId,access,ownerMode=false,ownerOverview,actions}:Props){
 ...
 const nav:[View,string,string][]=[["home","⌂","Dashboard"],["classes","▣",ownerMode?"Classrooms":"Courses"],["people","●","People"],["content","◇","Content"],["support","?","Support"],["systems","⚙","Admin"]];
 ...
 return <section className={styles.shell}>
  ...
  <main className={`${styles.main} ${secondaryOpen?"":styles.secondaryCollapsed}`}><header className={styles.header}><div><p>{ownerMode?"Owner":"Administration"} Portal</p><h1>{nav.find(item=>item[0]===view)?.[2]}</h1></div><div className={styles.headerActions}><button type="button" className={styles.secondaryToggle} aria-expanded={secondaryOpen} onClick={()=>setSecondaryOpen(v=>!v)}>{secondaryOpen?"Hide tools":"Show tools"}</button><span className={styles.badge}>{ownerMode?"OWNER":"ADMIN"}</span></div></header>
   ...
   <section className={styles.content}>
    ...
    {view==="support"&&tool==="requests"&&canEditContent&&<AdminOfficeRequestManager accessToken={accessToken} userId={userId} access={access}/>} {view==="support"&&tool==="tickets"&&canModerate&&<AdminSupportTicketManager accessToken={accessToken} userId={userId}/>} 
