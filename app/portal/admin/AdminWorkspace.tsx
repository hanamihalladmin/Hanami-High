...
type Tool="overview"|"status"|"announcements"|"academics"|"homerooms"|"homeroomSchedule"|"schedule"|"deletion"|"operations"|"exams"|"directory"|"ids"|"passes"|"narrative"|"moderation"|"events"|"campusOps"|"opportunities"|"applications"|"approvals"|"roadmap"|"lore"|"requests"|"tickets"|"roleplay"|"continuity"|"roadmapOps"|"transit"|"city"|"community";
...
export default function AdminWorkspace({accessToken,userId,access,ownerMode=false,ownerOverview,actions}:Props){
 ...
 const toolMap:Record<View,[Tool,string][]>= {home:[["overview","Overview"],["status","School Status"],["announcements","Announcements"]],classes:[["academics","Courses & Sections"],["homerooms","Homerooms"],["schedule","Schedule Blocks"],["deletion","Removal Tools"],["operations","Academic Operations"],["exams","Exams"]],people:[["directory","Directory"],["ids","School IDs"],["passes","Hall Passes"],["narrative","Narrative Review"],["moderation","Moderation"]],content:[["events","Events"],["campusOps","Campus Operations"],["opportunities","Opportunities"],["applications","Applications & PA"],["approvals","Approvals"],["roadmap","Roadmap"],["lore","Lore & Canon"]],support:[["requests","Office Requests"],["tickets","Support Tickets"]],systems:[["roleplay","Roleplay Systems"],["continuity","Continuity"],["roadmapOps","Roadmap Ops"],["transit","Transit"],["city","City Lifecycle"],["community","Community Ops"]]};
 ...
 return <section className={styles.shell}>
  ...
  <main className={styles.main}><header className={styles.header}><div><p>{ownerMode?"Owner":"Administration"} Portal</p><h1>{nav.find(item=>item[0]===view)?.[2]}</h1></div><span className={styles.badge}>{ownerMode?"OWNER":"ADMIN"}</span></header>
   ...
   <section className={styles.content}>
    ...
    {view==="classes"&&tool==="academics"&&access.site_admin&&<AdminAcademicManager accessToken={accessToken}/>} {view==="classes"&&tool==="homerooms"&&access.site_admin&&<AdminHomeroomManager accessToken={accessToken}/>} {view==="classes"&&tool==="homeroomSchedule"&&ownerMode&&<OwnerHomeroomScheduleEditor accessToken={accessToken}/>} {view==="classes"&&tool==="schedule"&&access.site_admin&&<AdminScheduleBlockManager accessToken={accessToken}/>} {view==="classes"&&tool==="deletion"&&access.site_admin&&<AdminDeletionManager accessToken={accessToken}/>} {view==="classes"&&tool==="operations"&&access.site_admin&&<AdminOperationsExpansionPanel accessToken={accessToken} userId={userId}/>} {view==="classes"&&tool==="exams"&&access.site_admin&&<AdminCompetitionExamManager accessToken={accessToken} userId={userId}/>} 
    ...
    {view==="content"&&tool==="events"&&canEditContent&&<AdminEventManager accessToken={accessToken} userId={userId} access={access}/>} {view==="content"&&tool==="campusOps"&&access.site_admin&&<AdminCampusOperationsPanel accessToken={accessToken}/>} {view==="content"&&tool==="opportunities"&&canEditContent&&<AdminOpportunityManager accessToken={accessToken} userId={userId}/>} {view==="content"&&tool==="applications"&&access.site_admin&&<AdminApplicationReviewPanel accessToken={accessToken} userId={userId}/>} {view==="content"&&tool==="approvals"&&canEditContent&&<AdminContentApprovalPanel accessToken={accessToken}/>} {view==="content"&&tool==="roadmap"&&canEditContent&&<AdminRoadmapManager accessToken={accessToken} userId={userId}/>} {view==="content"&&tool==="lore"&&canEditContent&&<AdminLoreCanonEditor accessToken={accessToken}/>} 
