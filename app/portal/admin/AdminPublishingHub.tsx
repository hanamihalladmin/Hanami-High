...
const cards:Card[]=[
 ...
 {key:"club_manage",title:"Clubs",description:"Manage school-owned club recruitment publishing authority without falling back to the legacy broad staff-management check.",route:{view:"content",tool:"campusOps"},action:"Open campus operations"},
...
export default function AdminPublishingHub({accessToken,siteAdmin=false}:{accessToken:string;siteAdmin?:boolean}){
 ...
 return <section className={styles.hub} aria-label="School Publishing Hub">
  ...
  <div className={styles.grid}>{cards.map(card=>{const allowed=Boolean(capabilities?.[card.key]);const campusBlocked=card.key==="club_manage"&&!siteAdmin;return <article className={styles.card} key={card.key}><div className={styles.cardHead}><h3>{card.title}</h3><span className={`${styles.badge} ${allowed?"":styles.badgeOff}`}>{allowed?"AUTHORIZED":"NO ACCESS"}</span></div><p>{card.description}</p><div className={styles.actions}>{card.href?allowed?<a href={card.href}>{card.action}</a>:<button type="button" disabled>{card.action}</button>:<button type="button" disabled={!allowed||campusBlocked} onClick={()=>card.route&&routeTo(card.route)}>{campusBlocked&&allowed?"Site admin UI required":card.action}</button>}</div></article>})}</div>
