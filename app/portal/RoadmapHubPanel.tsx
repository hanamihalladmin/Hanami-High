...
type Channel={id:string;title:string;description:string;channel_key:string};
type Membership={channel_id:string};
...
export default function RoadmapHubPanel({accessToken,characterId,role}:Props){
 ...
 const [channels,setChannels]=useState<Channel[]>([]);const [memberships,setMemberships]=useState<Membership[]>([]);const [feedback,setFeedback]=useState<Feedback[]>([]);const [votes,setVotes]=useState<Vote[]>([]);const [templates,setTemplates]=useState<Template[]>([]);
 ...
 const completed=new Set(progress.map(row=>row.item_id));const joined=new Set(entries.filter(row=>row.status!=="cancelled").map(row=>row.sheet_id));const testerJoined=new Set(memberships.map(row=>row.channel_id));const voted=new Set(votes.map(row=>row.item_id));
 ...
 const load=useCallback(async()=>{try{
  const requests=[
   ...
   fetch(`${SUPABASE_URL}/rest/v1/tester_release_channels?select=id,title,description,channel_key&is_active=eq.true&order=created_at.desc`,{headers:headers(accessToken)}),
   fetch(`${SUPABASE_URL}/rest/v1/tester_release_memberships?select=channel_id`,{headers:headers(accessToken)}),
  ...
  const setters=[setItems,setProgress,setLines,setStops,setLore,setCanon,setSignups,setEntries,setHours,setShifts,setMentors,setChannels,setMemberships,setFeedback,setVotes,setTemplates] as Array<(value:any)=>void>;
 ...
 async function toggleTester(channel:Channel){if(!userId)return;const active=testerJoined.has(channel.id);const endpoint=active?`${SUPABASE_URL}/rest/v1/tester_release_memberships?channel_id=eq.${channel.id}&user_id=eq.${userId}`:`${SUPABASE_URL}/rest/v1/tester_release_memberships`;const response=await fetch(endpoint,{method:active?"DELETE":"POST",headers:headers(accessToken,{"Content-Type":"application/json"}),body:active?undefined:JSON.stringify({channel_id:channel.id,user_id:userId})});setNotice(response.ok?(active?"Left tester channel.":"Joined tester channel."):"Tester membership could not be changed.");if(response.ok)void load();}
 ...
 <article style={card}><h5>Tester Release Channel</h5>{channels.length?channels.map(channel=><div key={channel.id} style={{marginBottom:8}}><strong>{channel.title}</strong><small style={{display:"block"}}>{channel.description}</small><button type="button" onClick={()=>toggleTester(channel)}>{testerJoined.has(channel.id)?"Leave preview channel":"Join preview channel"}</button></div>):<p>No preview channels are open.</p>}</article>
