...
export default function DashboardShell({character,accessToken}:Props){
  ...
  return <section className={styles.dashboard} aria-labelledby="dashboard-title">
    ...
    <div className={styles.notice}><strong>LIVE DASHBOARD</strong><span>{isStudent?"Student Schedule, Coursework, Campus Activities, Hanami Messages, Profile & Privacy, Hanami Profiles, and Profile Studio are connected to live Supabase data.":"Faculty Schedule, Course Management, Grading, Student Advising, Hanami Messages, Profile & Privacy, Hanami Profiles, and Profile Studio are connected to live Supabase data."}</span></div>
    ...
    <div className={styles.quickbar}><strong>{isStudent?"STUDENT QUICK LINKS":"FACULTY QUICK LINKS"}</strong><span>{isStudent?"Classes • Assignments • Calendar • Campus • Messages • Profile Studio":"Classes • Rosters • Assignments • Calendar • Messages • Profile Studio"}</span></div>
  </section>;
}
