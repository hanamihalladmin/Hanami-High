...
export default function DashboardShell({character,accessToken,onLogout}:Props){
 ...
 return <section className={styles.canvasShell} aria-labelledby="dashboard-title">
   <aside className={styles.globalNav} aria-label={`${isStudent?"Student":"Faculty"} global navigation`}>
    <a className={styles.logoButton} href="../../" aria-label="Hanami High home"><img src="../../hanami-high-portal-icon.png" alt=""/></a>
    <nav className={styles.primaryNav}>{nav.map(([key,icon,label])=><button key={key} type="button" className={view===key?styles.navActive:""} onClick={()=>setView(key)} title={label}><span className={styles.navIcon} aria-hidden="true">{key==="profile"?(avatarUrl?<img src={avatarUrl} alt=""/>:character.display_name.slice(0,1).toUpperCase()):icon}</span><b>{label}</b>{key==="messages"&&<MessageNotificationBadge accessToken={accessToken} characterId={character.id}/>}</button>)}</nav>
    <div className={styles.navBottom}>{!isStudent&&nurseEligible&&<button type="button" onClick={()=>setNurseMode(true)}><span aria-hidden="true">✚</span><b>Nurse</b></button>}<a href="../"><span aria-hidden="true">↔</span><b>Switch</b></a><button type="button" onClick={onLogout}><span aria-hidden="true">⇥</span><b>Logout</b></button></div>
   </aside>
   <main className={styles.main}>
    ...
    {view==="home"&&<div className={styles.dashboardView}><section className={styles.dashboardMain}><div className={styles.pageIntro}><h2>Dashboard</h2><p>Your courses, announcements, and school activity.</p></div><SchedulePanel accessToken={accessToken} characterId={character.id} role={character.role}/><SchoolNoticesPanel accessToken={accessToken}/></section><aside className={styles.dashboardAside}><DailySchoolBulletinPanel accessToken={accessToken} characterId={character.id}/><SchoolStatusPanel accessToken={accessToken}/></aside></div>}
