...
type Notice={id:string;title:string;body:string;category:"general"|"event"|"urgent";featured:boolean;published_at:string|null};
...
export default function SchoolNoticesPanel({accessToken}:Props){
 ...
 return <section className={styles.panel} aria-labelledby="school-notices-title"><div className={styles.heading}><div><p className="eyebrow">HANAMI SCHOOL NETWORK</p><h4 id="school-notices-title">School Notices</h4></div><span>LIVE CMS</span></div><div className={styles.status} aria-live="polite">{status}</div>{items.length>0&&<div className={styles.list}>{items.map(item=><article key={item.id} className={`${styles.item} ${item.category==="urgent"?styles.urgent:""}`}><div><p className="eyebrow">{item.category.toUpperCase()}{item.featured?" • FEATURED":""}</p><h5>{item.title}</h5>{item.published_at&&<time>{new Intl.DateTimeFormat("en-US",{month:"short",day:"numeric",year:"numeric",timeZone:"Asia/Tokyo"}).format(new Date(item.published_at))}</time>}</div><p>{item.body}</p></article>)}</div>}</section>;
}
