...
import YearbookPublishingDesk from "./YearbookPublishingDesk";
...
export default function YearbookPage(){
 ...
 const load=useCallback(async()=>{try{const headers={apikey:K};const [yr,cr]=await Promise.all([fetch(`${U}/rest/v1/yearbook_profiles?select=character_id,portrait_url,quote,clubs_sports,awards,most_likely_to,approved&approved=eq.true&order=updated_at.desc`,{headers,cache:"no-store"}),fetch(`${U}/rest/v1/characters?select=id,display_name,handle,role&role=eq.student`,{headers,cache:"no-store"})]);if(!yr.ok||!cr.ok)throw new Error();const [y,c]=await Promise.all([yr.json() as Promise<Yearbook[]>,cr.json() as Promise<Character[]>]);setRows(y);setCharacters(c);setStatus(`${y.length} approved student page${y.length===1?"":"s"}.`)}catch{setStatus("Approved yearbook pages are not publicly available yet.")}},[]);
 useEffect(()=>{void load();const refresh=()=>void load();window.addEventListener("hanami-yearbook-published",refresh);return()=>window.removeEventListener("hanami-yearbook-published",refresh)},[load]);
